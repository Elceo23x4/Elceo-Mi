...
export const POST = withApiErrorBoundary(async (request: Request) => {
  ...
  const body = unwrapValidation(validateAdminBillingOccurredAtRequest(await parseJsonBody(request, { maxBytes: 64 * 1024 })));
  const actor = securityActorFromVerifiedPrincipal(principal, 'admin');
  const security = await requireSecurityDecision({ request, routePath: '/api/admin/billing/cancel-at-period-end', method: 'POST', actionKind: 'admin_write', actor, subjectId: body.subjectId, requestBody: body });
  if (!security.ok) return security.response;
  try {
    const subscription = await getBillingRuntime().cancelAtPeriodEnd('user', body.subjectId, body.occurredAt);
    ...
    await auditInternalMutation({ actor, subjectId: body.subjectId, actionKind: 'admin_write', routePath: '/api/admin/billing/cancel-at-period-end', method: 'POST', request, idempotencyKey: security.idempotencyKey });
