...
export const POST = withApiErrorBoundary(async (request: Request) => {
  ...
  const body = unwrapValidation(validateAdminBillingChangePlanRequest(await parseJsonBody(request, { maxBytes: 64 * 1024 })));
  const actor = securityActorFromVerifiedPrincipal(principal, 'admin');
  const security = await requireSecurityDecision({ request, routePath: '/api/admin/billing/change-plan', method: 'POST', actionKind: 'admin_write', actor, subjectId: body.subjectId, requestBody: body });
  ...
  try {
    ...
    await auditInternalMutation({ actor, subjectId: body.subjectId, actionKind: 'admin_write', routePath: '/api/admin/billing/change-plan', method: 'POST', request, idempotencyKey: security.idempotencyKey });
