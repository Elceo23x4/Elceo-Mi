...
export const POST = withApiErrorBoundary(async (request: Request) => {
  ...
  const security = await requireSecurityDecision({ request, routePath: '/api/admin/billing/past-due', method: 'POST', actionKind: 'admin_write', actor, subjectId: body.subjectId, requestBody: body });
  if (!security.ok) return security.response;
  try {
    const subscription = await getBillingRuntime().markPastDue('user', body.subjectId, body.occurredAt);
    ...
    await auditInternalMutation({ actor, subjectId: body.subjectId, actionKind: 'admin_write', routePath: '/api/admin/billing/past-due', method: 'POST', request, idempotencyKey: security.idempotencyKey });
