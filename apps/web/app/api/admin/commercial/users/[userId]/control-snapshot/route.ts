...
export const GET = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ userId: string }> }) => { requireInternalRouteAccess(request); const access = await requireFeatureAccess('admin.read', { request }); if (!access.ok) return access.response; const { userId } = await context.params; return jsonSuccess({ targetUserId: userId, snapshot: getSuperAdminCommercialControlSnapshot(), stepUpReadinessState: 'fixture_verified_for_tests' as const }); });
