...
export const POST = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ userId: string }> }) => {
  ...
  try {
    const result = await giftFocusPlanToUser({ actorSuperAdminId: access.subject.userId, targetUserId: userId, duration, reasonCode: 'commercial_support', operatorNote: typeof body.operatorNote === 'string' ? body.operatorNote : '', stepUpVerification: step.value, idempotencyKey: security.idempotencyKey, requestedAt: new Date().toISOString() });
