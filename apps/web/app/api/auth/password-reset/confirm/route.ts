...
export async function POST(request:Request){
  ...
  try{const body=await parseJsonBody(request,{maxBytes:8*1024}) as {token?:unknown;password?:unknown};if(typeof body.token!=='string'||typeof body.password!=='string')throw new Error('invalid');const result=await credentialService.confirmReset(body.token,body.password);return result==='reset'?NextResponse.json({reset:true}):NextResponse.json({error:'invalid_or_expired_token'},{status:400});}
