...
export async function POST(request: Request) {
  ...
  try {
    ...
    const suppliedKey=body.idempotencyKey??request.headers.get('idempotency-key');if(!validKey(suppliedKey))throw new Error('payment_idempotency_key_required');const businessIdempotencyKey=suppliedKey;const provider=body.provider??(process.env.PAYMENT_PROVIDER_MODE==='production_provider'?null:'stripe');if(!provider)throw new Error('payment_provider_selection_required');const currency=(body.currency??'').toUpperCase();if(!currency)throw new Error('commercial_price_not_configured');const price=await resolveActiveCommercialPrice({planCode:'focus_plan',billingInterval:interval,currency});
