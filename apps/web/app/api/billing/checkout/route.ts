...
export async function POST(request: Request) {
  ...
  try {
    ...
    const suppliedKey=body.idempotencyKey??request.headers.get('idempotency-key');if(!validKey(suppliedKey))throw new Error('payment_idempotency_key_required');const businessIdempotencyKey=suppliedKey,repo=internalPaymentRuntime.repository,existing=await repo.findByBusinessIdempotency(session.user.id,businessIdempotencyKey);const requestedProvider=body.provider??(process.env.PAYMENT_PROVIDER_MODE==='production_provider'?null:'stripe');if(!requestedProvider)throw new Error('payment_provider_selection_required');if(existing&&(existing.provider!==requestedProvider||(body.currency&&existing.currency!==body.currency.toUpperCase())||existing.billingInterval!==interval))throw new Error('payment_idempotency_input_mismatch');const provider=(existing?.provider??requestedProvider) as 'stripe'|'korapay';const currency=existing?.currency??(body.currency??'').toUpperCase();if(!currency)throw new Error('commercial_price_not_configured');const activePrice=existing?null:await resolveActiveCommercialPrice({planCode:'focus_plan',billingInterval:interval,currency});const price=existing?{id:existing.commercialPriceVersionId!,amountMinor:existing.amount,currency:existing.currency}:activePrice!;
    ...
    const capability=await assertProviderCapability({provider,environment:process.env.APP_ENV==='production'?'production':'sandbox',rail:body.rail??(provider==='stripe'?'card':'checkout'),currency,recurring:provider==='stripe'});
    ...
    if (providerMode === 'sandbox_provider' || providerMode === 'production_provider') {
      ...
      const created = await repo.createOrReuseOperation({ subjectUserId: session.user.id, targetPlan: 'focus_plan', billingInterval: interval, amount:price.amountMinor, currency:price.currency, commercialPriceVersionId:price.id, quotedProviderProductReference:existing?.quotedProviderProductReference??(provider==='stripe'?process.env.STRIPE_PRODUCT_ID_FOCUS_PLAN:undefined), businessIdempotencyKey, provider });
      ...
      if (shouldResumeProviderCheckout(operation)) {
        try {
          const configuredBase=process.env.NEXT_PUBLIC_APP_BASE_URL;let origin:string;try{origin=new URL(configuredBase??'').origin}catch{throw new Error('canonical_application_base_url_required')}if(providerMode==='production_provider'&&!origin.startsWith('https://'))throw new Error('canonical_application_base_url_required');
          if(provider==='korapay'){let continueCheckout=true;
            const secret=process.env.KORAPAY_SECRET_KEY;if(!secret)throw new Error('korapay_secret_required');
            const adapter=new KoraPayAdapter(secret);if(!capability.moneyCapability)throw new Error('korapay_money_capability_unverified');if(created.reused){try{await adapter.retrieve(operation.providerIdempotencyKey,capability.moneyCapability);const reconciled=await reconcileProviderPaymentOperation(repo,operation.internalPaymentOperationId,{kora:adapter,moneyCapability:capability.moneyCapability});operation=reconciled.operation??operation;checkoutUrl=null;continueCheckout=false}catch(error){if((error as {status?:number}).status!==404)throw error;}}if(continueCheckout){const result=await adapter.initialize({amountMinor:operation.amount,moneyCapability:capability.moneyCapability,currency:operation.currency,reference:operation.providerIdempotencyKey,notificationUrl:`${origin}/api/billing/webhook/korapay`,redirectUrl:`${origin}/settings?billing=return`,customer:{name:session.user.name??'ELCEO customer',email:session.user.email??''},plan:'focus_plan',interval,operationId:operation.internalPaymentOperationId,channels:body.rail?[body.rail]:undefined});
            ...
            const result = await adapter.createCheckoutOrPaymentSession({ subjectUserId: session.user.id, targetPlan: 'focus_plan', providerProductReference:operation.quotedProviderProductReference, billingInterval: interval, amount: operation.amount, currency: operation.currency, providerIdempotencyKey: operation.providerIdempotencyKey, operationId: operation.internalPaymentOperationId, successUrl: `${origin}/settings?billing=sandbox_success`, cancelUrl: `${origin}/settings?billing=sandbox_cancel` });
          ...
          const normalized = normalizeProviderError(providerError,provider);
    ...
    const result = await internalPaymentRuntime.checkout({ subjectUserId: session.user.id, targetPlan: 'focus_plan', billingInterval: interval, amount:price.amountMinor, currency:price.currency, commercialPriceVersionId:price.id, businessIdempotencyKey, outcome: fakeOutcomesEnabled ? body.fakeProviderOutcome : undefined });
