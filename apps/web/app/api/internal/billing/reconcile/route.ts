...
import { internalPaymentRuntime, reconcileStripePaymentOperation, StripeSandboxPaymentProviderAdapter } from '@elceo/application-state';
...
export const POST = withApiErrorBoundary(async (request: Request) => {
  ...
  try {
    const payment = body.sourceEventId.startsWith('ipo_') && body.providerKind==='stripe' ? await reconcileStripePaymentOperation(internalPaymentRuntime.repository,new StripeSandboxPaymentProviderAdapter(),body.sourceEventId) : body.sourceEventId.startsWith('ipo_') ? await internalPaymentRuntime.reconcile({ operationId: body.sourceEventId, businessIdempotencyKey: body.sourceEventId, subjectUserId: body.subjectId }) : { status: 'not_found' as const };
