...
import { internalPaymentRuntime, reconcileProviderPaymentOperation } from '@elceo/application-state';
...
export const POST = withApiErrorBoundary(async (request: Request) => {
  ...
  try {
    const payment = body.sourceEventId.startsWith('ipo_') ? await reconcileProviderPaymentOperation(internalPaymentRuntime.repository,body.sourceEventId) : { status: 'not_found' as const };
