...
export const POST = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ caseId: string }> }) => {
  ...
  try {
  const updated = await getApplicationStateRuntime().journal.adjustExecution('user', subject.subjectId, caseId, mapJournalAdjustRequest(patch), { actorKind: 'user', actorId: subject.userId });
