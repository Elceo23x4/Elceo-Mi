...
import { mapJournalCancelRequest } from '@elceo/application-state';
...
export const POST = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ caseId: string }> }) => {
  ...
  const patch = unwrapValidation(validateJournalCancelRequest(await parseJsonBody(request, { maxBytes: 64 * 1024 })));
  ...
  try {
  const updated = await getTenantApplicationStateRuntime(subject).journal.cancelCase('user', subject.subjectId, caseId, mapJournalCancelRequest(patch), { actorKind: 'user', actorId: subject.userId });
