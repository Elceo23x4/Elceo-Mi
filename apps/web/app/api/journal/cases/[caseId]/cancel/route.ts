...
export const POST = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ caseId: string }> }) => {
  ...
  try {
  const updated = await getApplicationStateRuntime().journal.cancelCase('user', subject.subjectId, caseId, mapJournalCancelRequest(patch), { actorKind: 'user', actorId: subject.userId });
