...
export const POST = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ caseId: string }> }) => {
  ...
  const patch = unwrapValidation(validateJournalCloseRequest(await parseJsonBody(request, { maxBytes: 64 * 1024 })));
  ...
  try {
  const updated = await getTenantApplicationStateRuntime(subject).journal.closeCase('user', subject.subjectId, caseId, mapJournalCloseRequest(patch), { actorKind: 'user', actorId: subject.userId });
