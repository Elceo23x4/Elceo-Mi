...
import { mapJournalExecuteRequest } from '@elceo/application-state';
import { validateJournalExecuteRequest } from '@elceo/schemas';

export const POST = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ caseId: string }> }) => {
  const subject = await requireAuthenticatedSubject();
  const { caseId } = await context.params;
  const patch = unwrapValidation(validateJournalExecuteRequest(await parseJsonBody(request, { maxBytes: 64 * 1024 })));
  const actor = { actorKind: 'user' as const, actorId: subject.userId, subjectId: subject.subjectId };
  const security = await requireSecurityDecision({ request, routePath: '/api/journal/cases/[caseId]/execute', method: 'POST', actionKind: 'journal_case_lifecycle', actor, subjectId: subject.subjectId, requestBody: patch });
  if (!security.ok) return security.response;
  try {
  const updated = await getTenantApplicationStateRuntime(subject).journal.markExecuted('user', subject.subjectId, caseId, mapJournalExecuteRequest(patch), { actorKind: 'user', actorId: subject.userId });
    ...
    await auditInternalMutation({ actor, subjectId: subject.subjectId, actionKind: 'journal_case_lifecycle', routePath: '/api/journal/cases/[caseId]/execute', method: 'POST', request, idempotencyKey: security.idempotencyKey });
