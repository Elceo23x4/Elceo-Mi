...
import { mapJournalPartialCloseRequest } from '@elceo/application-state';
import { validateJournalPartialCloseRequest } from '@elceo/schemas';

export const POST = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ caseId: string }> }) => {
  const subject = await requireAuthenticatedSubject();
  const { caseId } = await context.params;
  const patch = unwrapValidation(validateJournalPartialCloseRequest(await parseJsonBody(request, { maxBytes: 64 * 1024 })));
  const actor = { actorKind: 'user' as const, actorId: subject.userId, subjectId: subject.subjectId };
  const security = await requireSecurityDecision({ request, routePath: '/api/journal/cases/[caseId]/partial-close', method: 'POST', actionKind: 'journal_case_lifecycle', actor, subjectId: subject.subjectId, requestBody: patch });
  if (!security.ok) return security.response;
  try {
  const updated = await getTenantApplicationStateRuntime(subject).journal.markPartiallyClosed('user', subject.subjectId, caseId, mapJournalPartialCloseRequest(patch), { actorKind: 'user', actorId: subject.userId });
    ...
    await auditInternalMutation({ actor, subjectId: subject.subjectId, actionKind: 'journal_case_lifecycle', routePath: '/api/journal/cases/[caseId]/partial-close', method: 'POST', request, idempotencyKey: security.idempotencyKey });
