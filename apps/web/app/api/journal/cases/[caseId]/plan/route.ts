...
export const POST = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ caseId: string }> }) => {
  ...
  try {
    const updated = await getApplicationStateRuntime().journal.planCase('user', subject.subjectId, caseId, mapJournalPlanRequest(patch), { actorKind: 'user', actorId: subject.userId });
