...
import { mapJournalPlanRequest } from '@elceo/application-state';
...
export const POST = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ caseId: string }> }) => {
  ...
  const patch = unwrapValidation(validateJournalPlanRequest(await parseJsonBody(request, { maxBytes: 64 * 1024 })));
  ...
  try {
    const updated = await getTenantApplicationStateRuntime(subject).journal.planCase('user', subject.subjectId, caseId, mapJournalPlanRequest(patch), { actorKind: 'user', actorId: subject.userId });
