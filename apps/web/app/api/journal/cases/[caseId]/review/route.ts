...
import { mapJournalReviewRequest } from '@elceo/application-state';
...
export const POST = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ caseId: string }> }) => {
  ...
  try {
  const updated = await getTenantApplicationStateRuntime(subject).journal.reviewCase('user', subject.subjectId, caseId, mapJournalReviewRequest(patch), { actorKind: 'user', actorId: subject.userId });
    ...
    await auditInternalMutation({ actor, subjectId: subject.subjectId, actionKind: 'journal_case_lifecycle', routePath: '/api/journal/cases/[caseId]/review', method: 'POST', request, idempotencyKey: security.idempotencyKey });
