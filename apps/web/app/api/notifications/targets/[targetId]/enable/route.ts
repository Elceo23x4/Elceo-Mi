...
export const POST = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ targetId: string }> }) => {
  ...
  const security = await requireSecurityDecision({ request, routePath: '/api/notifications/targets/[targetId]/enable', method: 'POST', actionKind: 'notification_target_write', actor, subjectId: subject.subjectId, requestBody: { targetId } });
  if (!security.ok) return security.response;
  try {
    const target = await getTenantNotificationRuntimes(subject).management.enableTargetForSubject('user', subject.subjectId, targetId);
