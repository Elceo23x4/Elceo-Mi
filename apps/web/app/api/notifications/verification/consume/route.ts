...
export const POST = withApiErrorBoundary(async (request: Request) => {
  ...
  try {
    const result = await getTenantNotificationRuntimes(subject).verification.consumeTargetVerificationForSubject('user', subject.subjectId, body.targetId, body.token);
