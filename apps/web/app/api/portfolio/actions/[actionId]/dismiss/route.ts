...
export const POST = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ actionId: string }> }) => {
  ...
  try {
    const action = await getTenantApplicationStateRuntime(subject).portfolio.dismissActionItem('user', subject.subjectId, actionId, new Date().toISOString(), { actorKind: 'user', actorId: subject.userId });
