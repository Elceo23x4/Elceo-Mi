...
export const GET = withApiErrorBoundary(async (_request: Request, context: { params: Promise<{ actionId: string }> }) => {
  ...
  const replay = await getTenantApplicationStateRuntime(subject).portfolio.getPortfolioEntityReplay('user', subject.subjectId, 'action_item', actionId);
...
export const PATCH = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ actionId: string }> }) => {
  ...
  try {
    const action = await getTenantApplicationStateRuntime(subject).portfolio.updateActionItem('user', subject.subjectId, actionId, patch, { actorKind: 'user', actorId: subject.userId });
