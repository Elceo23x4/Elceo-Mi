...
export const POST = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ positionId: string }> }) => {
  ...
  unwrapValidation(validatePositionCancelRequest(await parseJsonBody(request, { maxBytes: 64 * 1024 })));
    const security = await requireSecurityDecision({ request, routePath: '/api/portfolio/positions/[positionId]/cancel', method: 'POST', actionKind: 'portfolio_position_write', actor, subjectId: subject.subjectId, requestBody: {} });
  if (!security.ok) return security.response;
  try {
const position = await getTenantApplicationStateRuntime(subject).portfolio.cancelPosition('user', subject.subjectId, positionId, { actorKind: 'user', actorId: subject.userId });
    ...
    await auditInternalMutation({ actor, subjectId: subject.subjectId, actionKind: 'portfolio_position_write', routePath: '/api/portfolio/positions/[positionId]/cancel', method: 'POST', request, idempotencyKey: security.idempotencyKey });
