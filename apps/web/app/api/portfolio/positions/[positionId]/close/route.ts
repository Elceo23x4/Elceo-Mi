...
export const POST = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ positionId: string }> }) => {
  ...
  const body = unwrapValidation(validatePositionCloseRequest(await parseJsonBody(request, { maxBytes: 64 * 1024 })));
  ...
  try {
const position = await getTenantApplicationStateRuntime(subject).portfolio.closePosition('user', subject.subjectId, positionId, body.closedAt, body, { actorKind: 'user', actorId: subject.userId });
