...
export const POST = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ positionId: string }> }) => {
  ...
  const body = unwrapValidation(validatePositionOpenRequest(await parseJsonBody(request, { maxBytes: 64 * 1024 })));
    const security = await requireSecurityDecision({ request, routePath: '/api/portfolio/positions/[positionId]/open', method: 'POST', actionKind: 'portfolio_position_write', actor, subjectId: subject.subjectId, requestBody: body });
  if (!security.ok) return security.response;
  try {
const position = await getTenantApplicationStateRuntime(subject).portfolio.openPosition('user', subject.subjectId, positionId, body.openedAt, body, { actorKind: 'user', actorId: subject.userId });
    ...
    await auditInternalMutation({ actor, subjectId: subject.subjectId, actionKind: 'portfolio_position_write', routePath: '/api/portfolio/positions/[positionId]/open', method: 'POST', request, idempotencyKey: security.idempotencyKey });
