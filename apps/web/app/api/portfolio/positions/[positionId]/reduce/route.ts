...
export const POST = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ positionId: string }> }) => {
  ...
  const body = unwrapValidation(validatePositionReduceRequest(await parseJsonBody(request, { maxBytes: 64 * 1024 })));
  ...
  try {
const position = await getTenantApplicationStateRuntime(subject).portfolio.reducePosition('user', subject.subjectId, positionId, body, { actorKind: 'user', actorId: subject.userId });
