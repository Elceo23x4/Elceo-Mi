...
export const POST = withApiErrorBoundary(async (request: Request, context: { params: Promise<{ entryId: string }> }) => {
  ...
  const body = unwrapValidation(validateWatchlistThesisHealthRequest(await parseJsonBody(request, { maxBytes: 64 * 1024 })));
  ...
  try {
    const entry = await getTenantApplicationStateRuntime(subject).portfolio.changeWatchlistThesisHealth('user', subject.subjectId, entryId, body.thesisHealth, { actorKind: 'user', actorId: subject.userId });
