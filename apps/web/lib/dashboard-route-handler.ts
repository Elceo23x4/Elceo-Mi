...
export function createDashboardGetHandler(dependencies:{authenticate:()=>Promise<AuthState>;readDashboard:(asset:string,signal:AbortSignal)=>Promise<DashboardChartWorkspaceViewModel|null>;readKickOff?:(asset:string,signal:AbortSignal,features:{evidenceScore:boolean;macroHeadlines:boolean})=>Promise<KickOffDashboardViewModelV1|null>;admit:typeof withDashboardReadAdmission;authorizeCommercial?:(userId:string)=>Promise<boolean>;resolveCommercialAccess?:(userId:string)=>Promise<CommercialAccess|'kick_off'>}){
  return async function dashboardGet(_:Request,context:{params:Promise<{asset:string}>}){try{
    ...
    const resolved=dependencies.resolveCommercialAccess?await dependencies.resolveCommercialAccess(subject):dependencies.authorizeCommercial&&!(await dependencies.authorizeCommercial(subject))?'denied':'focus_plan',access=resolved==='kick_off'?{access:'kick_off' as const,features:{evidenceScore:true,macroHeadlines:true}}:resolved;
    ...
    if(access!=='focus_plan'&&!dependencies.readKickOff)return NextResponse.json({error:'Dashboard data unavailable'},{status:500});
