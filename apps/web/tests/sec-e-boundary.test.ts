...
import { parseJsonBody, parsePositiveInt, withApiErrorBoundary } from '../lib/server/api/http';
...
export async function runSecEBoundaryTests(){
 ...
 for(const unsupported of [undefined,()=>1,Symbol('x'),1n,NaN,Infinity,-Infinity])assert.throws(()=>canonicalJson(unsupported));
 const cyclic:Record<string,unknown>={};cyclic.self=cyclic;assert.throws(()=>canonicalJson(cyclic));
 ...
 await assert.rejects(parseJsonBody(new Request('http://x',{method:'POST',headers:{'content-length':'1'},body:'{"aa":1}'}),{maxBytes}),/payload_too_large/);
 await assert.rejects(parseJsonBody(new Request('http://x',{method:'POST',body:'not-json'}),{maxBytes:32}),/bad_request:invalid_json/);
 let canceled=false;const stream=new ReadableStream<Uint8Array>({start(controller){controller.enqueue(new TextEncoder().encode('{"tooLong":true}'));},cancel(){canceled=true;}});
 await assert.rejects(parseJsonBody(new Request('http://x',{method:'POST',body:stream,duplex:'half'} as RequestInit & {duplex:'half'}),{maxBytes:4}),/payload_too_large/);assert.equal(canceled,true);
 const oversizedHandler=withApiErrorBoundary(async(request:Request)=>{await parseJsonBody(request,{maxBytes:4});return Response.json({ok:true});});const oversizedResponse=await oversizedHandler(new Request('http://x',{method:'POST',body:'{"a":1}'}));assert.equal(oversizedResponse.status,413);assert.deepEqual(await oversizedResponse.json(),{ok:false,error:{code:'payload_too_large',message:'Payload too large'}});
