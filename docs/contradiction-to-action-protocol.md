...
Assessment history is a linear immediate → confirmation → follow-through supersession chain. Each stage has a distinct persisted evaluation, assessment hash, contradiction-input identity, decision identity, and deterministic transition link. SQL tests execute previous-row locking, recursive ancestor lookup, and transition insertion; evidence, parent, and transition failures roll back transaction-local changes. Analog tests reject cross-event, post-cutoff, and future-stage retrievals and prove that sparse/no-history/outcome-only context cannot alter direct evidence hashes or state selection.
