...
Finalizable review or escalation requires `finalizationReadiness.ready === true` with no unresolved readiness reason. A readiness-false record waits even when its direct contradiction would otherwise review or escalate; finalizable aligned evidence also waits. Only ready, final, resolved evidence archives. Strictly increasing stage order makes cycles structurally unreachable through the public repository, while the recursive ancestor query remains defense in depth.

Focused fixtures attach complete `historicalOutcomeContext` objects for rejected, reversed, and invalidated historical outcomes. Those attachments may alter analog context only; direct state, contradiction-input identity, matrix identity, and invalidation identity remain unchanged.
