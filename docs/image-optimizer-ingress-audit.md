...
| Temporary `elceo-image-cert-*` fixtures | Certification scripts only | Test-controlled raw pixels | PNG/JPEG/WebP; 64×48; malformed/SVG rejection cases | Yes, production `/_next/image` and Sharp | Local CI only | Exact metadata, format, hashes and rejection status | Supported Next 16.3.0 to Sharp 0.35.3 contract |
