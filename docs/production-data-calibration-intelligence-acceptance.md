...
Every persisted record has an immutable domain timestamp: manifest `generatedAt`, certification `certifiedAt`, split/coverage/risk/configuration/trial/rollback/run `createdAt`, lifecycle `selectedAt`, and case `frozenAt`. SQL never substitutes database time. Coverage observations are canonical entities whose exact hash-derived ID binds policy, cell, dataset, split, run family, observed evidence and creation time.

Empirical criterion scopes are executable. Empty asset, event-class, horizon, or segment arrays explicitly mean “all”; non-empty arrays filter the frozen holdout cases before diagnostics and minimum-sample evaluation. Each result records its matched sample count, scoped value, state and reason. An empty, unsupported, or undersized required slice is `insufficient_evidence`, never a fallback to an aggregate metric.

Production-chain machinery execution is distinct from production empirical certification. Tests execute the actual IFP-1 through IFP-7 adapter against persisted, explicitly fixture-class reasoning records, while the public acceptance service rejects that fixture corpus before holdout exposure. This machinery proof cannot create production acceptance. Structural `not_applicable` decisions additionally require an empirical-policy criterion to explicitly list the authoritative structural decision IDs used by its matching coverage cells; an unrelated or inferred exemption remains insufficient evidence.

Atomic acceptance persistence also validates the declared evidence graph before writing: run family, rollback, exact case hashes, coverage, risks, and unique artifact links must agree. Dataset and configuration links use persisted identities, while split and certification links additionally resolve and reconcile their canonical semantic IDs. A transaction cannot make an incoherent bundle durable merely because its individual records are canonical.
