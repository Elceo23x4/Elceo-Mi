...
## PGS-2C owner failure evidence

The internal PGS-1 owner executor now carries its completed structured gate result when execution throws. This preserves the claimed reservation snapshot and confirmed `RELEASED`, `COMMIT_REQUIRED_UNKNOWN_OUTCOME`, or settlement-unconfirmed state without exposing the provider exception or reopening a public uncached live path. PGS-2 publishes only a finite sanitized coordination reason, while the caller receives truthful evidence of the provider-control lifecycle.

Pre-transmission ownership loss therefore reports `settled_released`; timeout, ownership loss, or adapter failure after invocation reports `settled_unknown_outcome` unless settlement itself is unconfirmed. A still-eligible stale response may replace the payload but never replaces this owner accounting evidence. Real-Redis acceptance also checks exact once-only rate, quota, cost, and concurrency behavior for ambiguous and late execution-claim recovery and managed timeout. Production live remains blocked, and PGS-3 has not started.
...
## PGS-5 — scale economics, inbound protection, and load acceptance

PGS-5 closes user-read execution: dashboard GET reads the existing `DashboardChartWorkspaceViewModel` only and missing state remains a 503; it never bootstraps ingestion. Canonical cognition consumers use a read-only Redis-pointer/PostgreSQL-artifact composition. A bounded per-process LRU (entry and serialized-byte ceilings), exact-identity integrity validation, and cold single-flight bound durable reads without changing `freshUntil` or pointer authority. A cached artifact is usable only when Redis names that exact identity; a new uncached pointer is unavailable during PostgreSQL failure.

Authenticated inbound dashboard reads are admitted before retrieval by one Redis TIME Lua operation. The trusted, hashed policy is supplied by server configuration; production numbers are not embedded. Subject keys use bounded digests and per-subject Redis Cluster hash tags. Fixed-window consumption is retained after admission, while concurrency is an expiring unique-owner lease with exact-token renewal/release. Redis or policy failure is a finite fail-closed 503; limits are 429. Browser IP headers never participate. CDN/WAF remains responsible for volumetric DDoS, bot, and network/IP controls.

The `test:provider-scale-acceptance` gate uses real `REDIS_URL` and `DATABASE_URL`, seeds a valid immutable cognition artifact, reads its real Redis pointer, and performs 100,000 reads with bounded concurrency. It reports provider, cognition, and scheduler deltas (all structurally zero because the reader cannot construct those dependencies), PostgreSQL reads, cache behavior, and multi-instance/admission results. This is CI structural amplification evidence, not dollar savings, a production p95/p99 SLO, staging sizing evidence, WAF validation, or “production load certified” status. `load:pgs5-staging` requires a real authenticated endpoint and emits machine-readable evidence; it has no simulation mode.

The existing persisted `chartViewModelByAsset` remains the dashboard contract and the legacy/compatibility ingestion and chart-intelligence persistence path remains its current producer. PGS-5 does not invent a canonical scheduled chart-view producer; migrating that producer to canonical materialization is an explicit future integration gap. Production live remains disabled.
