...
The existing persisted `chartViewModelByAsset` remains the dashboard contract and ingestion persistence remains its current producer. PGS-5 does not invent a canonical scheduled chart-view producer; migrating that producer to canonical materialization is an explicit future integration gap. Production live remains disabled.
