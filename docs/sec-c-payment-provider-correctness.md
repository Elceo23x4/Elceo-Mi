...
Recurring Stripe webhooks authenticate first, then retrieve provider truth before the lifecycle transaction. Lifecycle event IDs, timestamps, canceled-state protection, and recurring periods form an idempotency domain separate from the initial payment grant; renewals can advance `app_billing_subscriptions` even when the initial grant effect already exists. Refund, reversal, and chargeback events use their negative ledger/entitlement paths and never refresh a paid period. Different checkout gesture keys that collide with an unresolved-intention constraint fail as `payment_unresolved_intention_exists`; same-key retries read the immutable quote and Product snapshot before consulting current pricing. Korapay correlation is reference-first and provider query metadata is optional defense-in-depth. Calendar-paid periods clamp at UTC month ends. The Stripe portal is available in guarded sandbox or production mode only for the canonical Stripe subscription/customer and uses the canonical configured return URL.
