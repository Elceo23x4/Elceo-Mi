...
| `app_notification_targets`, `app_notification_subscriptions`, `app_notification_verifications` | direct `subject_kind` + `subject_id` |
...
| ingestion, market evidence, reasoning/cognition reference data, provider events, delivery/outbox processing, commercial/admin/audit/security controls | internal/admin/system authority, not tenant-runtime tables |

Migration `0058` enables RLS and adds `USING` plus `WITH CHECK` policies. The runtime role must be NOSUPERUSER/NOBYPASSRLS and must not own tables. A request transaction calls `set_config('app.authenticated_subject_id', subjectId, true)` through `withTenantDbTransaction` using an already verified server subject. Transaction-local state prevents pool leakage. Owners/migration roles retain an explicit broader administration path; the runtime role receives only separately provisioned grants and cannot bypass RLS. Role credentials are operational secrets and are not stored here.

## Acceptance evidence

`npm run test:sec-f` executes runtime adversarial telemetry, Sentry, token, CSRF and environment tests plus static CSP/RLS assertions. The sentinel is embedded in innocent strings, nested values and stack URLs and must not survive serialization. `npm run test:sec-f-postgres` creates an ephemeral NOSUPERUSER/NOBYPASSRLS login, applies migrations as owner, then runs all isolation assertions through the restricted login: own access; foreign raw-SQL SELECT/UPDATE/DELETE and forged INSERT denial; child revision/verification denial; and rollback/next-transaction context reset. CI runs both commands after SEC-E and before the production build. This is repository acceptance evidence only, not live or staging certification.
