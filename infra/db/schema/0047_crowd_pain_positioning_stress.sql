...
CREATE TABLE IF NOT EXISTS positioning_evidence (
 positioning_evidence_id text PRIMARY KEY, policy_version text NOT NULL CHECK(policy_version='positioning-stress-v1'), source_payload_id text NOT NULL,
 provider text NOT NULL, asset text NOT NULL, source_market_code text NOT NULL, report_kind text NOT NULL, cohort text NOT NULL,
 ...
 CONSTRAINT positioning_evidence_time_check CHECK(observed_at<=available_at AND (published_at IS NULL OR published_at<=available_at) AND available_at<=evidence_cutoff_at),
 UNIQUE(provider,source_market_code,report_kind,cohort,observed_at)
);
