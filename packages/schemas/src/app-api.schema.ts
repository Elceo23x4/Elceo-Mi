import type {
  ...
  InternalScheduledIngestionReplayRequest,
...
const VALID_SCHEDULED_INGESTION_RUN_MODE = ['dry_run_fixture','staging_live','production_live'] as const;
...
export function validateInternalScheduledIngestionReplayRequest(i: unknown): SchemaValidationResult<InternalScheduledIngestionReplayRequest> { const parsed=validateObject(i); if(!parsed.ok) return {ok:false,errors:(parsed as {ok:false;errors:string[]}).errors}; const v=parsed.value; const errors:string[]=[]; if(!(isNonEmptyString(v.runId) && isSafeIdentifier(v.runId))) errors.push('runId is invalid'); if(!(v.replayMode===undefined||v.replayMode===null||isEnumValue(v.replayMode,VALID_SCHEDULED_INGESTION_RUN_MODE))) errors.push('replayMode is invalid'); if(!(v.startedAt===undefined||v.startedAt===null||isIsoDateString(v.startedAt))) errors.push('startedAt must be ISO timestamp'); if(v.production_live!==undefined||v.productionLive!==undefined) errors.push('production_live is not allowed'); if(v.providerApiKey!==undefined||v.apiKey!==undefined||v.tiingoApiKey!==undefined) errors.push('provider API keys are not allowed'); return errors.length?{ok:false,errors}:{ok:true,value:{runId:v.runId as string,replayMode:(v.replayMode as InternalScheduledIngestionReplayRequest['replayMode']) ?? null,startedAt:(v.startedAt as string|null|undefined) ?? null}}; }
