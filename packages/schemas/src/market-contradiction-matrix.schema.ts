...
function validateWarnings(value: unknown, errors: string[], path: string): void { if (!Array.isArray(value) || value.some((x) => !isEnumValue(x, MARKET_CONTRADICTION_WARNINGS))) errors.push(`${path} must contain valid contradiction warnings`); }
