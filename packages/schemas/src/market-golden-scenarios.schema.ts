...
export function validateMarketGoldenScenarioRuleSetSnapshot(input: unknown): SchemaValidationResult<MarketGoldenScenarioRuleSetSnapshot> { if (!isObjectRecord(input)) return fail(['must be object']); const e: string[] = []; if (!Array.isArray(input.rules) || input.rules.length === 0) e.push('rules'); else input.rules.forEach((x) => { const r = validateMarketGoldenScenarioRule(x); if (r.ok === false) e.push(...r.errors); }); if (!Array.isArray(input.launchTradableAssets) || input.launchTradableAssets.length !== 12 || input.launchTradableAssets.includes('dxy') || input.launchTradableAssets.includes('vix')) e.push('launchTradableAssets'); if (!Array.isArray(input.diagnosticAssets) || input.diagnosticAssets.length !== 2 || !input.diagnosticAssets.includes('dxy') || !input.diagnosticAssets.includes('vix')) e.push('diagnosticAssets'); if (!Array.isArray(input.reasoningAssets) || input.reasoningAssets.length !== 14) e.push('reasoningAssets'); { const rr=validateExpectedMarketReasoningModuleReadiness(input.readiness,'golden_scenarios','readiness.'); if(rr.ok===false)e.push(...rr.errors); } return e.length ? fail(e) : pass(input as MarketGoldenScenarioRuleSetSnapshot); }
