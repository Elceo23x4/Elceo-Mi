...
const confidenceModifiers = ['improves','reduces','cautious','contradiction','not_applicable'] as const;
...
function pendingFalse(input: Record<string, unknown>, errors: string[]): void { if (input.complete !== false) errors.push('complete must remain false'); const p = input.pending; if (!isObjectRecord(p) || p.liveProviderActivation !== true || p.empiricalBacktesting !== true || p.productionDataCalibration !== true) errors.push('pending flags must remain true'); }
...
export function validateMarketGoldenScenarioFixture(input: unknown): SchemaValidationResult<MarketGoldenScenarioFixture> {
  ...
  if (!isObjectRecord(price)) e.push('priceReactionExpectation');
    ...
    if (!isEnumValue(price.confidenceModifier, confidenceModifiers)) e.push('priceReactionExpectation.confidenceModifier');
