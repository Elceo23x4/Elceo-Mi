...
export type AdminProviderCapabilitySummary = { generatedAt:string; notificationProviders:Array<{providerKind:string; configured:boolean; enabled:boolean; capabilityStatus:AdminComponentStatus; reasons:string[]}>; ingestionProviders:Array<{providerName:string; category:string; credentialPresent:boolean; configured:boolean; liveDisabled:boolean; stagingLiveAuthorized:boolean; stagingLiveValidated:boolean; productionLiveBlocked:boolean; enabled:boolean; capabilityStatus:AdminComponentStatus; reasons:string[]}> };
