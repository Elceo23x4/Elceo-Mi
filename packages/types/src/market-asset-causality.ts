...
export type MarketAssetCausalityGap = { gapId: string; asset: MarketAssetCausalityAsset | 'all'; readinessCategory: 'live_provider_integration'|'empirical_validation'|'production_calibration'; status: Exclude<MarketAssetCoverageStatus, 'partially_covered'|'mapped_contract_valid'>; description: string; };
export type MarketAssetCausalityCoverageReport = { generatedAt: string; launchTradableAssetCount: number; diagnosticAssetCount: number; representedReasoningAssetCount: number; assetSupportRoles: Record<MarketAssetCausalityAsset, MarketAssetSupportRole>; readiness: Readiness; representedAssets: MarketAssetCausalityAsset[]; missingAssets: MarketAssetCausalityAsset[]; duplicateAssets: MarketAssetCausalityAsset[]; gapCount: number; gaps: MarketAssetCausalityGap[]; /** @deprecated Means all readiness dimensions are satisfied; canonical readiness is structured. */ complete: boolean; notes: string[]; };
