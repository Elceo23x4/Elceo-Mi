...
import type { MarketProviderReliabilityResult } from './market-provider-reliability';
...
export const MARKET_CONFIDENCE_CALIBRATION_TIER_BANDS = [
  { tier:'very_low', min:0, max:25 },
  { tier:'low', min:25, max:45 },
  { tier:'medium', min:45, max:65 },
  { tier:'high', min:65, max:80 },
  { tier:'very_high', min:80, max:100 }
] as const satisfies readonly { tier: MarketConfidenceCalibrationTier; min: number; max: number }[];
export function marketConfidenceTierForScore(score: number): MarketConfidenceCalibrationTier { const clamped = Math.max(0, Math.min(100, score)); const lastIndex = MARKET_CONFIDENCE_CALIBRATION_TIER_BANDS.length - 1; const band = MARKET_CONFIDENCE_CALIBRATION_TIER_BANDS.find((entry, index) => clamped >= entry.min && (index === lastIndex ? clamped <= entry.max : clamped < entry.max)); if (!band) throw new Error('malformed_market_confidence_calibration_tier_bands'); return band.tier; }
...
export type MarketConfidenceCalibrationOptions = { priceReactionAvailable?: boolean; priceReaction?: MarketPriceReactionResult; providerReliabilitySupplied?: boolean; providerReliability?: MarketProviderReliabilityResult; providerReliabilityResults?: MarketProviderReliabilityResult[]; sourceIndependenceVerified?: boolean; eventSensitive?: boolean; macroDriven?: boolean; fxDiagnosticPath?: boolean };
...
export type MarketConfidenceCalibrationInput = {
  ...
  warnings: string[];
  reasonCodes: string[];
  options?: MarketConfidenceCalibrationOptions;
};
