...
export function marketConfidenceTierForScore(score: number): MarketConfidenceCalibrationTier { const clamped = Math.max(0, Math.min(100, score)); const band = MARKET_CONFIDENCE_CALIBRATION_TIER_BANDS.find((entry) => clamped >= entry.min && clamped <= entry.max); if (!band) throw new Error('malformed_market_confidence_calibration_tier_bands'); return band.tier; }
