...
export type SecurityIdempotencyResponseRecord = { idempotencyKey: string; actionKind: SecurityActionKind; actorKind: SecurityActorKind; actorId: string; requestHash: string; responseHash: string; httpStatus: number; responseJson: string; completedAt: string; expiresAt: string; metadataJson: string };
export type SecurityIdempotencyReplayReason = 'completed_response_found' | 'no_completed_response' | 'request_hash_mismatch' | 'expired' | 'not_found';
export type SecurityIdempotencyReplayResult = { replayable: boolean; idempotencyKey: string; httpStatus: number | null; responseJson: string | null; responseHash: string | null; reason: SecurityIdempotencyReplayReason };
