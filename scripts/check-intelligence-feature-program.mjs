...
const ifp4Surfaces = [read('services/reasoning/src/tests/market-cleanliness.test.ts'), read('services/reasoning/src/market-cleanliness/contracts.ts'), read('services/reasoning/src/market-cleanliness/evaluator.ts'), read('services/reasoning/src/market-cleanliness/sql-context-repository.ts'), read('services/reasoning/src/market-cleanliness/sql-repository.ts'), read('services/reasoning/src/persistence/memory-reasoning-repository.ts'), read('services/reasoning/src/persistence/sql-reasoning-repository.ts'), read('infra/db/schema/0045_market_cleanliness_ranking.sql'), read('scripts/test-ifp4-postgres.mjs')].join('\n');
for (const label of ['IFP4 effective-reliability downgrade','IFP4 clean canonical rejection','IFP4 neutral/inline coherence','IFP4 confirmation T+6 mutation neutrality','IFP4 session-state consistency','IFP4 typed analog point-in-time safety','IFP4 analog source references','IFP4 canonical persistence composition','IFP4 standalone migration execution','IFP4 actual service-driven PostgreSQL clean rejection','IFP4 SQL context/evaluation lineage rejection','cleanliness_score_out_of_range','IFP4 no-advice validator execution']) if (!ifp4Surfaces.includes(label)) fail(`IFP-4 executable acceptance surface missing: ${label}`);
