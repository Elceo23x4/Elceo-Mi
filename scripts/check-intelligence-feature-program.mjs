...
for(const executable of ['narrativeAsOf', 'evidenceQualifiedPersistenceScore', 'observed_interval', 'source_freshness_is_non_authoritative', 'MemoryNarrativeDecayRepository', 'SqlNarrativeDecayRepository', 'IFP5 superseded old version terminal', 'IFP5 revision baseline cognition cutoff', 'IFP5 semantic checkpoint exact dedupe', 'IFP5 proportional related coverage ratio', 'IFP5 memory SQL event-version parity visibility rule', 'IFP5 PostgreSQL lifecycle matrix superseded old version terminal', 'IFP5 SQL concurrent identical save', 'IFP5 SQL bad-lineage rollback'] )if(!ifp5Surface.includes(executable))fail(`IFP-5 executable surface missing: ${executable}`);
