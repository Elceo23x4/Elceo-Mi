...
for(const executable of ['narrativeAsOf','evidenceQualifiedPersistenceScore','observed_interval','source_freshness_is_non_authoritative','equalOrEarlierAnalogStage','MemoryNarrativeDecayRepository','SqlNarrativeDecayRepository','IFP5 actual service-driven PostgreSQL clean rejection','IFP5 terminal expired narrative','IFP5 cumulative epoch peak','IFP5 missing related is not conflict','IFP5 revision reversal supersedes prior version','IFP5 future event interpretation rejection','IFP5 future observation rejection','IFP5 future cognition rejection','IFP5 provenance trust-promotion rejection','IFP5 real IFP-2 analog service integration','IFP5 PostgreSQL concurrent identical save','IFP5 PostgreSQL concurrent conflicting save','IFP5 PostgreSQL transactional rollback'])if(!ifp5Surface.includes(executable))fail(`IFP-5 executable surface missing: ${executable}`);
