...
let retrieved = null;
if (!operation.providerCheckoutSessionReference && !operation.providerPaymentReference) refuse('sandbox execution not completed: provider paid state not confirmed');
...
if (!retrieved || !['succeeded','refunded','partially_refunded'].includes(retrieved.status)) refuse('sandbox execution not completed: provider paid state not confirmed');
if (retrieved.providerSessionReference && retrieved.providerSessionReference !== operation.providerCheckoutSessionReference) refuse('sandbox smoke invariant failed: retrieved provider session mismatch');
if (retrieved.providerPaymentReference && operation.providerPaymentReference && retrieved.providerPaymentReference !== operation.providerPaymentReference) refuse('sandbox smoke invariant failed: retrieved provider payment mismatch');
const beforePaidTruthCounts = await rt.counts();
...
if (normalized.metadataOperationId !== operation.internalPaymentOperationId) refuse('sandbox smoke invariant failed: webhook operation metadata mismatch');
if (normalized.metadataProviderIdempotencyKey !== operation.providerIdempotencyKey) refuse('sandbox smoke invariant failed: webhook provider idempotency metadata mismatch');
if (normalized.providerSessionReference !== operation.providerCheckoutSessionReference) refuse('sandbox smoke invariant failed: webhook session reference mismatch');
if (normalized.providerPaymentReference !== operation.providerPaymentReference) refuse('sandbox smoke invariant failed: webhook payment reference mismatch');
const firstWebhook = await rt.webhook({ eventId: normalized.providerEventId ?? normalized.safeRedactedPayloadChecksum, kind:'success', operationId: normalized.metadataOperationId, providerPaymentReference: normalized.providerPaymentReference, providerCheckoutSessionReference: normalized.providerSessionReference, payload:{ normalized, providerTruthStatus:retrieved.status } });
const countsAfterFirst = await rt.counts();
if (countsAfterFirst.ledger !== beforePaidTruthCounts.ledger + 1 || countsAfterFirst.entitlements !== beforePaidTruthCounts.entitlements + 1) refuse('sandbox smoke invariant failed: paid provider truth did not create exactly one effect');
...
const summary = {status:'sandbox_smoke_e2e_passed', providerKind:'stripe', sandboxOnly:true, productionLive:false, operationId:operation.internalPaymentOperationId, providerIdempotencyKeyChecksum:stableProviderChecksum(operation.providerIdempotencyKey), providerSessionReference:redactRef(operation.providerCheckoutSessionReference), providerPaymentReference:redactRef(operation.providerPaymentReference), retrievedStatus:retrieved.status, webhookAccepted:firstWebhook.duplicate === false, duplicateWebhookDeduped:countsAfterFirst.ledger === countsAfterDuplicate.ledger && countsAfterFirst.entitlements === countsAfterDuplicate.entitlements, reconciliationStatus:reconciled.status, ledgerEffects:countsAfterReconcile.ledger, entitlementEffects:countsAfterReconcile.entitlements};
if (/(?:sk|rk|whsec)_(?:live|test)?_?[A-Za-z0-9]{12,}/.test(JSON.stringify(summary))) refuse('sandbox smoke invariant failed: secret-like value in summary');
