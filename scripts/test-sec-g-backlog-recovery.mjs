...
function runWorker(domain,prefix,mode,index){
 const child=spawn(process.execPath,['scripts/sec-g-backlog-worker.mjs'],{cwd:process.cwd(),env:{...process.env,SEC_G_BACKLOG_DOMAIN:domain,SEC_G_BACKLOG_PREFIX:prefix,SEC_G_BACKLOG_MODE:mode,SEC_G_BACKLOG_TOKEN:`${mode}-${index}-${randomUUID()}`,SEC_G_BACKLOG_LIMIT:'10',SEC_G_HOLD_MS:'10000'},stdio:['ignore','pipe','pipe']});
 let buffer='';const event=new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(new Error(`sec_g_${domain}_${mode}_worker_timeout`)),10000);child.stdout.on('data',chunk=>{buffer+=chunk.toString();const nl=buffer.indexOf('\n');if(nl<0)return;clearTimeout(timer);try{resolve(JSON.parse(buffer.slice(0,nl)));}catch(error){reject(error);}});child.once('error',reject);child.stderr.on('data',chunk=>{if(process.env.SEC_G_DEBUG==='1')process.stderr.write(chunk);});});
 return{child,event};
}
async function waitExit(child){return await new Promise(resolve=>child.once('exit',(code,signal)=>resolve({code,signal})));}
...
async function execute(domain){
 ...
 return{exactGitSha:process.env.SEC_G_HEAD_SHA??null,scenario:`${domain}-meaningful-backlog-process-death-recovery`,environment:'github-actions-test',startedAt,endedAt:new Date().toISOString(),initialRowCount:count,initialStateCounts:initial,ownerClaimedCount:claimed.rows.length,killedOwnerPid:holder.child.pid,killedSignal:killed.signal,killPoint,takeoverDelayMs:takeoverStarted-new Date(killPoint).getTime(),replacementWorkers:drained.map(item=>({pid:item.pid,claimed:item.claimed,completed:item.completed,batches:item.batches,generations:item.generations})),staleTransitionRows:stale,finalStateCounts:final,maxSuccessfulAttemptsPerRow:maxAttempts,elapsedRecoveryDrainMs:Date.now()-started,invariants:{expiredClaimsRecoverable:true,successorGenerationAdvanced:successorAdvanced,staleOwnerFenced:Object.values(stale).every(value=>value===0),noRowsLost:final.reduce((sum,row)=>sum+Number(row.count),0)===count,noDuplicateCanonicalCompletion:maxAttempts===1,backlogFullyDrained:terminalCount===count}};
}
try{const ingestion=await execute('ingestion');const notification=await execute('notification');assert(Object.values(ingestion.invariants).every(Boolean));assert(Object.values(notification.invariants).every(Boolean));await writeFile(`${dir}/ingestion-backlog-recovery.json`,JSON.stringify(ingestion,null,2));await writeFile(`${dir}/notification-backlog-recovery.json`,JSON.stringify({...notification,providerSpecificRecoveryEvidence:'notification-recovery.json'},null,2));console.log(JSON.stringify({ingestion:ingestion.invariants,notification:notification.invariants}));}finally{await db.end();}
