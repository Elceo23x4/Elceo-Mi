...
const scenario=async(name,operation)=>{try{return await timed(name,operation);}catch(error){console.error(JSON.stringify({scenario:name,operation:'canonical PostgreSQL ownership acceptance',sqlState:error?.code??null,message:error?.message??String(error),expected:'all ownership and fencing assertions pass'}));throw error;}};
...
async function opsFencing(){
 ...
 const acquire=async(index,acquiredAt,expiresAt)=>{const leaseId=`sec-g-ops-lease-${index}-${randomUUID()}`;await pool.query(`UPDATE app_ops_job_leases SET lease_state='expired' WHERE job_kind=$1 AND scope_kind=$2 AND scope_key=$3 AND lease_state='acquired' AND expires_at <= $4`,[jobKind,scopeKind,scopeKey,acquiredAt]);const result=await pool.query(`INSERT INTO app_ops_job_leases(lease_id,job_kind,scope_kind,scope_key,lease_state,acquired_at,expires_at,released_at,holder_id,created_at,owner_token,generation) VALUES($1,$2,$3,$4,'acquired',$6,$7,NULL,$5,$6,$1,(SELECT COALESCE(MAX(generation),0)+1 FROM app_ops_job_leases WHERE job_kind=$2 AND scope_kind=$3 AND scope_key=$4)) ON CONFLICT DO NOTHING RETURNING lease_id,owner_token,generation`,[leaseId,jobKind,scopeKind,scopeKey,`holder-${index}`,acquiredAt,expiresAt]);return result.rows[0]??null;};
 ...
 const stale=await pool.query(`UPDATE app_ops_job_leases SET lease_state='released',released_at=$4 WHERE lease_id=$1 AND owner_token=$2 AND generation=$3 AND lease_state='acquired' RETURNING lease_id`,[first.lease_id,first.owner_token,first.generation,iso(350)]);
 const current=await pool.query(`UPDATE app_ops_job_leases SET lease_state='released',released_at=$4 WHERE lease_id=$1 AND owner_token=$2 AND generation=$3 AND lease_state='acquired' RETURNING lease_id`,[successor.lease_id,successor.owner_token,successor.generation,iso(400)]);
 assert.equal(stale.rowCount,0);assert.equal(current.rowCount,1);
...
async function notificationInboxQueryCount(){
 const subjectId=`sec-g-inbox-owner-${randomUUID()}`,targetIds=Array.from({length:20},(_,i)=>`sec-g-inbox-target-${i}-${randomUUID()}`);
 await concurrent(targetIds.length,(i)=>pool.query(`INSERT INTO app_notification_targets(target_id,target_key,subject_kind,subject_id,channel,target_kind,status,label,address_json,created_at,updated_at) VALUES($1,$2,'user',$3,'in_app','in_app_user','active','SEC-G query-count',jsonb_build_object('value',$4::text),now(),now())`,[targetIds[i],`sec-g-inbox-key-${randomUUID()}`,subjectId,`sec-g-inbox-address-${i}`]));
 await concurrent(targetIds.length,(i)=>pool.query(`INSERT INTO app_notification_inbox(inbox_id,target_id,decision_id,decision_key,asset,timeframe,rule_key,headline,body,created_at,payload_json) VALUES($1,$2,$3,$4,'BTC/USD','H1','sec-g-query-count','SEC-G','SEC-G',now()-($5||' seconds')::interval,'{}')`,[`sec-g-inbox-${randomUUID()}`,targetIds[i],`sec-g-inbox-decision-${randomUUID()}`,`sec-g-inbox-decision-key-${randomUUID()}`,i]));
 ...
 const canonical=await pool.query(`SELECT i.inbox_id,i.target_id FROM app_notification_inbox i INNER JOIN app_notification_targets t ON t.target_id=i.target_id WHERE t.subject_kind=$1 AND t.subject_id=$2 AND i.archived_at IS NULL ORDER BY i.created_at DESC,i.inbox_id DESC LIMIT 100`,['user',subjectId]);canonicalQueries++;
...
try{
 const ingestion=await scenario('ingestion-postgres-claim-fencing',ingestionFencing);
 const scheduler=await scenario('scheduler-postgres-lease-fencing',schedulerFencing);
 const ops=await scenario('ops-postgres-scope-fencing',opsFencing);
 const inbox=await scenario('notification-inbox-executed-query-count',notificationInboxQueryCount);
 await Promise.all([
