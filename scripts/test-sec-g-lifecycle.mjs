...
async function run(mode,signal='SIGTERM',priorHandler=false){
  const startedAt=new Date().toISOString();
  const dir=await mkdtemp(join(tmpdir(),'sec-g-life-')),marker=join(dir,'marker');
  const child=spawn(process.execPath,['scripts/sec-g-lifecycle-child.mjs'],{cwd:process.cwd(),env:{...process.env,SEC_G_LIFECYCLE_MARKER:marker,SEC_G_DRAIN_MODE:mode,SEC_G_PRIOR_SIGNAL_HANDLER:priorHandler?'1':'0',ELCEO_SHUTDOWN_GRACE_MS:'75'}});
...
const delegated=await run('success','SIGTERM',true);
...
assert.match(graceful.events,/postgres-stop\nsampler-stop\nsampler-drain-start\nsampler-drain-complete\npostgres-drain-start\npostgres-drain-complete/);
assert.equal(delegated.code,0);assert.match(delegated.events,/sampler-drain-complete[\s\S]*postgres-drain-complete[\s\S]*next-handler/);
assert.equal(failure.code,1);
assert.match(failure.events,/sampler-drain-failed\npostgres-drain-start\npostgres-drain-complete/);
assert.equal(timeout.code,1);
assert.equal(hardKill.signal,'SIGKILL');
assert(!hardKill.events.includes('drain-start'));

const gracefulEvidence={exactGitSha:head,scenario:'node-process-sigterm-bounded-drain',environment,...graceful,delegated,failure,timeout,invariants:{stopNewWorkObserved:/sampler-stop/.test(graceful.events),drainStarted:/sampler-drain-start/.test(graceful.events),drainCompleted:/sampler-drain-complete/.test(graceful.events),orderedBeforePostgres:graceful.events.indexOf('sampler-drain-complete')<graceful.events.indexOf('postgres-drain-start'),nextHandlerAfterElceoDrain:delegated.events.indexOf('postgres-drain-complete')<delegated.events.indexOf('next-handler'),remainingCleanupAfterFailure:/sampler-drain-failed[\s\S]*postgres-drain-complete/.test(failure.events),failureExitNonZero:failure.code===1,timeoutExitNonZero:timeout.code===1,exitZero:graceful.code===0},accepted:true};
