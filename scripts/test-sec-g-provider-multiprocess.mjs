...
function worker(namespace,index,counterPath,{barrier,region}={}){
 ...
 let buffer='',readyResolve,resultResolve,resultReject;const ready=new Promise((resolve,reject)=>{readyResolve=resolve;child.once('error',reject);}),result=new Promise((resolve,reject)=>{resultResolve=resolve;resultReject=reject;});
 ...
 child.stderr.on('data',chunk=>{if(process.env.SEC_G_DEBUG==='1')process.stderr.write(chunk);});child.once('exit',(code,signal)=>{if(code!==0&&code!==2)resultReject(new Error(`provider_worker_exit:${index}:${code}:${signal}`));});
...
const containerId=redisServiceContainer(),outageNamespace=`elceo:sec-g:provider:outage:${randomUUID()}`,outageCounter=`${dir}/.provider-outage-count`,barrier=`${dir}/.provider-outage-release`;
...
const stopped=spawnSync('docker',['stop','--time','0',containerId],{encoding:'utf8'});assert.equal(stopped.status,0,stopped.stderr);await writeFile(barrier,'release');const outageResults=await Promise.all(outageWorkers.map(item=>item.result));await Promise.all(outageWorkers.map(item=>new Promise(resolve=>item.child.once('exit',resolve))));const outageUpstream=await countLines(outageCounter);assert.equal(outageUpstream,0,'Redis authority loss must prevent unmanaged upstream execution');
...
await Promise.all([rm(successCounter,{force:true}),rm(outageCounter,{force:true}),rm(recoveryCounter,{force:true}),rm(barrier,{force:true})]);console.log(JSON.stringify(providerEvidence));
