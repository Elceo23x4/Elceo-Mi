...
const integrityGuard=await readFile(new URL('./assert-sec-g-source-integrity.mjs',import.meta.url),'utf8');
...
const invariants={
  ...
  adaptiveBuildPreparedExplicitly:/prepare:sec-g-adaptive/.test(exactHeadWorkflow),
  canonicalArtifactDirectory:/SEC_G_ARTIFACT_DIR:.*github\.workspace.*artifacts\/sec-g/.test(exactHeadWorkflow)&&/SEC_G_ARTIFACT_DIR:.*github\.workspace.*artifacts\/sec-g/.test(empiricalJob),
  fiveFileProvenance:['test-sec-g-adaptive-takeover.mjs','sec-g-adaptive-worker.mjs','assert-sec-g-source-integrity.mjs','verify-sec-g-artifacts.mjs','.github/workflows/sec-g-exact-head.yml'].every(path=>integrityGuard.includes(path)),
  sensitiveEvidenceScannedInBothWorkflows:/scan-sec-g-artifacts\.mjs/.test(empiricalJob)&&/scan-sec-g-artifacts\.mjs/.test(exactHeadWorkflow)
};
