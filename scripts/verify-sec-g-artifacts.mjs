...
assert.equal(files['sensitive-artifact-scan.json'].json.accepted,true,'sec_g_sensitive_artifact_scan_not_accepted');
...
assert.deepEqual([...integrityPhases.keys()].sort(),['after-checkout','before-adaptive','before-verification'],'sec_g_source_integrity_phase_inventory_mismatch');
for(const phaseName of ['after-checkout','before-adaptive','before-verification']){
 ...
 assert.equal(phase.observations?.length,5,`sec_g_source_integrity_observations_missing:${phaseName}`);
