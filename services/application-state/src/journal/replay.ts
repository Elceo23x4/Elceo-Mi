...
async function toReplayBundle(
  ...
  const revisions = await repository.listRevisionsForCase(caseRecord.caseId);
  return { caseRecord, caseData, revisions };
}

export async function getJournalCaseReplayById(caseId: string, repository: JournalCaseRepository): Promise<JournalCaseReplayBundle | null> {
  const caseRecord = await repository.getCaseById(caseId);
