...
import type {KoraMoneyCapability} from './kora-money';
import {fixedPaidPeriod} from '../billing/paid-period';
const environment=()=>((globalThis as any).process?.env??{}) as Record<string,string|undefined>;
export async function reconcileProviderPaymentOperation(repository:InternalPaymentRepository,operationId:string,deps?:{stripe?:StripeSandboxPaymentProviderAdapter;kora?:KoraPayAdapter;moneyCapability?:KoraMoneyCapability}){
 ...
 const reference=operation.providerTransactionReference??operation.providerPaymentReference??operation.providerIdempotencyKey;if(!reference)return {status:'unresolved' as const,operation,reason:'provider_reference_missing'};
 let truth;try{truth=await (deps?.kora??new KoraPayAdapter(env.KORAPAY_SECRET_KEY??'')).retrieve(reference,deps?.moneyCapability??{minorUnitExponent:-1,providerAmountEncoding:'decimal_major_json_number'});validateKoraTruth({reference,amountMinor:String(operation.amount),currency:operation.currency,plan:operation.targetPlan,interval:operation.billingInterval??'monthly',operationId},truth)}catch{return {status:'unresolved' as const,operation,reason:'provider_unavailable_or_mismatch'}}
 const period=fixedPaidPeriod(truth.paidAt??operation.currentPeriodStart??operation.createdAt,operation.billingInterval??'monthly');return repository.transaction(async tx=>{const locked=await tx.lockOperation(operationId);if(!locked)return {status:'not_found' as const,operation:null};const next=await tx.transition(operationId,locked.version,'succeeded',{reconciliationState:'matched',providerTransactionReference:reference,providerPaymentReference:null,subscriptionState:'active',currentPeriodStart:period.currentPeriodStart,currentPeriodEnd:period.currentPeriodEnd,lastProviderComparisonSnapshot:{reference,status:truth.status,amountMinor:truth.amountAccepted??truth.amountPaid??truth.amountExpected,currency:truth.currency}},'Kora provider-backed reconciliation matched');await tx.writeLedgerOnce(next,'payment_success');await tx.writeEntitlementOnce(next,'grant');await tx.appendAudit(operationId,'Kora provider truth inspected',{result:'matched'});return {status:'matched' as const,operation:next};});
}
