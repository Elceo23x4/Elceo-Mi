...
export function validateStripeCommercialIdentity(op:InternalPaymentOperation,truth:NormalizedProviderEvent):string|null{
...
export function validateStripeProviderTruth(op:InternalPaymentOperation,truth:NormalizedProviderEvent):string|null{
 ...
 const commercial=validateStripeCommercialIdentity(op,truth);if(commercial)return commercial;
 if(!truth.providerSubscriptionReference||truth.subscriptionState!=='active')return 'provider_subscription_not_current';
...
export async function reconcileStripePaymentOperation(repository:InternalPaymentRepository,retriever:StripeTruthRetriever,operationId:string,eventId?:string):Promise<StripeReconciliationResult>{
