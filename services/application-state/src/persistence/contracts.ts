...
export type PersistedSecurityIdempotencyRecord = {
  idempotencyKey: string; actionKind: import('@elceo/types').SecurityActionKind; actorKind: import('@elceo/types').SecurityActorKind; actorId: string; requestHash: string; responseHash: string | null; status: 'started' | 'completed' | 'failed'; firstSeenAt: string; lastSeenAt: string; expiresAt: string; metadataJson: string;
};

export type PersistedSecurityIdempotencyResponseRecord = {
  idempotencyKey: string; actionKind: import('@elceo/types').SecurityActionKind; actorKind: import('@elceo/types').SecurityActorKind; actorId: string; requestHash: string; responseHash: string; httpStatus: number; responseJson: string; completedAt: string; expiresAt: string; metadataJson: string;
};
export type PersistedSecurityRateLimitCounterRecord = {
  counterId: string; policyKey: string; actionKind: import('@elceo/types').SecurityActionKind; actorKind: import('@elceo/types').SecurityActorKind; actorId: string; subjectId: string | null; window: import('@elceo/types').SecurityRateLimitWindow; windowStart: string; windowEnd: string; count: number; updatedAt: string;
};
export type PersistedSecurityAuditEventRecord = {
  auditEventId: string; actorKind: import('@elceo/types').SecurityActorKind; actorId: string; subjectId: string | null; actionKind: import('@elceo/types').SecurityActionKind; decisionStatus: import('@elceo/types').SecurityDecisionStatus; blockReason: import('@elceo/types').SecurityBlockReason | null; routePath: string | null; method: string | null; ipHash: string | null; userAgentHash: string | null; idempotencyKey: string | null; metadataJson: string; occurredAt: string; createdAt: string;
};
export type SecurityIdempotencyRestartResult = { status: 'restarted'; record: PersistedSecurityIdempotencyRecord } | { status: 'not_failed'; record: PersistedSecurityIdempotencyRecord | null } | { status: 'hash_mismatch'; record: PersistedSecurityIdempotencyRecord };
export type SecurityIdempotencyRepository = { getIdempotencyRecord(idempotencyKey: string): Promise<PersistedSecurityIdempotencyRecord | null>; saveIdempotencyRecord(record: PersistedSecurityIdempotencyRecord): Promise<void>; restartFailedIdempotencyRecordAtomically(input: { idempotencyKey: string; requestHash: string; metadataJson: string; nowIso: string }): Promise<SecurityIdempotencyRestartResult>; completeIdempotencyRecord(idempotencyKey: string, responseHash: string, metadataJson: string, completedAt: string): Promise<void>; failIdempotencyRecord(idempotencyKey: string, metadataJson: string, failedAt: string): Promise<void>; cleanupExpiredIdempotencyRecords(asOfIso: string): Promise<number>; };
export type SecurityIdempotencyResponseRepository = { saveResponse(record: PersistedSecurityIdempotencyResponseRecord): Promise<void>; getResponse(idempotencyKey: string): Promise<PersistedSecurityIdempotencyResponseRecord | null>; cleanupExpiredResponses(asOfIso: string): Promise<number>; listResponsesForActor(actorKind: import('@elceo/types').SecurityActorKind, actorId: string, limit?: number): Promise<PersistedSecurityIdempotencyResponseRecord[]>; };
export type SecurityRateLimitRepository = { getCounter(policyKey: string, actorKind: import('@elceo/types').SecurityActorKind, actorId: string, subjectId: string | null, windowStart: string, windowEnd: string): Promise<PersistedSecurityRateLimitCounterRecord | null>; upsertCounter(record: PersistedSecurityRateLimitCounterRecord): Promise<void>; incrementCounter(params: { policyKey: string; actionKind: import('@elceo/types').SecurityActionKind; actorKind: import('@elceo/types').SecurityActorKind; actorId: string; subjectId: string | null; window: import('@elceo/types').SecurityRateLimitWindow; windowStart: string; windowEnd: string; updatedAt: string; }): Promise<PersistedSecurityRateLimitCounterRecord>; listCountersForActor(actorKind: import('@elceo/types').SecurityActorKind, actorId: string, limit?: number): Promise<PersistedSecurityRateLimitCounterRecord[]>; };
export type SecurityAuditEventRepository = { saveAuditEvent(record: PersistedSecurityAuditEventRecord): Promise<void>; listRecentAuditEventsForActor(actorKind: import('@elceo/types').SecurityActorKind, actorId: string, limit?: number): Promise<PersistedSecurityAuditEventRecord[]>; listRecentAuditEventsForSubject(subjectId: string, limit?: number): Promise<PersistedSecurityAuditEventRecord[]>; listRecentBlockedEvents(limit?: number): Promise<PersistedSecurityAuditEventRecord[]>; getSecurityRuntimeSummary(asOfIso?: string): Promise<import('@elceo/types').SecurityRuntimeSummary>; };

export type { SuperAdminStepUpRepository, StepUpPersistenceReadiness, StepUpPersistenceStatus } from './super-admin-step-up-repository';
