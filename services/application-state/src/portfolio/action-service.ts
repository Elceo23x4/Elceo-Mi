...
export class ActionService {
  ...
  private async load(subjectKind: 'user' | 'workspace' | 'ops', subjectId: string, actionId: string): Promise<PortfolioActionItem> {
    const row = await this.repository.getActionItemForSubject(subjectKind, subjectId, actionId);
  ...
  async linkAction(subjectKind: 'user' | 'workspace' | 'ops', subjectId: string, actionId: string, patch: Partial<Pick<PortfolioActionItem, 'linkedEntryId' | 'linkedPositionId' | 'linkedJournalCaseId' | 'linkedReasoningRunId' | 'linkedNotificationDecisionId'>>, actor: ActionActor): Promise<PortfolioActionItem> {
    const current = await this.load(subjectKind, subjectId, actionId);
  ...
  async updateActionItem(subjectKind: 'user' | 'workspace' | 'ops', subjectId: string, actionId: string, patch: Partial<Omit<PortfolioActionItem, 'actionId' | 'subjectKind' | 'subjectId' | 'createdAt'>>, actor: ActionActor): Promise<PortfolioActionItem> {
    const current = await this.load(subjectKind, subjectId, actionId);
    const next: PortfolioActionItem = { ...current, ...patch, updatedAt: actor.changedAt ?? nowIso() };
    return this.saveWithRevision(next, 'updated', actor);
  }

  async completeActionItem(subjectKind: 'user' | 'workspace' | 'ops', subjectId: string, actionId: string, completedAt: string, actor: ActionActor): Promise<PortfolioActionItem> {
    const current = await this.load(subjectKind, subjectId, actionId);
  ...
  async dismissActionItem(subjectKind: 'user' | 'workspace' | 'ops', subjectId: string, actionId: string, dismissedAt: string, actor: ActionActor): Promise<PortfolioActionItem> {
    const current = await this.load(subjectKind, subjectId, actionId);
