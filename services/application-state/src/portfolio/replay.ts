...
export async function getPortfolioEntityReplay(repository: PortfolioRepository, entityKind: PortfolioEntityKind, entityId: string): Promise<PortfolioEntityReplay | null> {
  let current: unknown;
  if (entityKind === 'watchlist_entry') {
    const row = await repository.getWatchlistEntryById(entityId);
    if (!row) return null;
    current = deserializeWatchlistEntry(row.entryJson);
  } else if (entityKind === 'position') {
    const row = await repository.getPositionById(entityId);
    if (!row) return null;
    current = deserializePositionRecord(row.positionJson);
  } else {
    const row = await repository.getActionItemById(entityId);
    if (!row) return null;
    current = deserializePortfolioActionItem(row.actionJson);
  }

  const revisions = await repository.listRevisionsForEntity(entityKind, entityId);
