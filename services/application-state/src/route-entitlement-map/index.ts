...
export const ROUTE_ENTITLEMENT_MAP = [
  ...
  { method:'GET', path:'/api/account/profile/social-identifiers', policy:'authenticated_basic', featureKey:null, sensitivity:'high', requiredGuard:'requireAuthenticatedSubject+assertRouteSubjectOwnership' },
  { method:'PATCH', path:'/api/account/profile/social-identifiers', policy:'authenticated_basic', featureKey:null, sensitivity:'high', requiredGuard:'requireAuthenticatedSubject+assertRouteSubjectOwnership' },
  ...
  { method:'POST', path:'/api/admin/commercial/users/[userId]/gift-focus-plan', policy:'admin_ops_required', featureKey:'super_admin.commercial_controls', sensitivity:'critical', requiredGuard:'requireInternalRouteAccess+requireFeatureAccess+requireSecurityDecision+step_up_required' },
  { method:'POST', path:'/api/admin/commercial/users/[userId]/retract-focus-gift', policy:'admin_ops_required', featureKey:'super_admin.commercial_controls', sensitivity:'critical', requiredGuard:'requireInternalRouteAccess+requireFeatureAccess+requireSecurityDecision+step_up_required' },
  { method:'POST', path:'/api/admin/commercial/users/[userId]/restrict', policy:'admin_ops_required', featureKey:'super_admin.commercial_controls', sensitivity:'critical', requiredGuard:'requireInternalRouteAccess+requireFeatureAccess+requireSecurityDecision+step_up_required' },
  { method:'GET', path:'/api/admin/commercial/users/[userId]/control-snapshot', policy:'admin_read_required', featureKey:'super_admin.commercial_controls', sensitivity:'critical', requiredGuard:'requireInternalRouteAccess+requireFeatureAccess' },
...
export const ROUTE_FAMILY_AUDIT_STATUS = {
  ...
  'account/profile': 'owner_runtime_guarded',
  ...
  'Super Admin commercial controls': 'helper_guarded',
