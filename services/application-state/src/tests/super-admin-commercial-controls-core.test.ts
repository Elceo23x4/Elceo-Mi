...
export async function runSuperAdminCommercialControlsCoreTests(): Promise<void> {
  const gift=giftFocusPlanToUser({actorSuperAdminId:'admin-1',targetUserId:'user-1',duration:'two_weeks',reasonCode:'commercial_support',operatorNote:'grant',stepUpVerification:stepUp,idempotencyKey:'id-1',requestedAt:'2026-05-15T00:00:00.000Z'});
  ...
  const giftMonth=giftFocusPlanToUser({actorSuperAdminId:'admin-1',targetUserId:'user-2',duration:'one_month',reasonCode:'commercial_support',operatorNote:'grant',stepUpVerification:stepUp,idempotencyKey:'id-2',requestedAt:'2026-05-15T00:00:00.000Z'});
  assert.equal(giftMonth.giftRecord?.endsAt,'2026-06-14T00:00:00.000Z');
  const blocked=giftFocusPlanToUser({actorSuperAdminId:'admin-1',targetUserId:'user-1',duration:'two_weeks',reasonCode:'commercial_support',operatorNote:'x',stepUpVerification:{...stepUp,status:'missing'},idempotencyKey:null,requestedAt:'2026-05-15T00:00:00.000Z'} as never);
  assert.equal(blocked.status,'blocked');
  const retracted=retractFocusPlanGift({actorSuperAdminId:'admin-1',targetUserId:'user-1',giftRecordId:gift.giftRecord!.giftRecordId,reasonCode:'operator_correction',operatorNote:'reverse',stepUpVerification:stepUp,idempotencyKey:'id-3',requestedAt:'2026-05-15T00:01:00.000Z'});
  assert.equal(retracted.status,'success');
  assert.equal(retracted.giftRecord?.status,'retracted');
  const restriction=restrictUserAccount({actorSuperAdminId:'admin-1',targetUserId:'user-3',restrictionKind:'banned',reasonCode:'policy_violation',operatorNote:'abuse',stepUpVerification:stepUp,idempotencyKey:'id-4',requestedAt:'2026-05-15T00:00:00.000Z'});
