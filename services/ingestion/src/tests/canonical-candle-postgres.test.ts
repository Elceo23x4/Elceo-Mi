...
async function findRepositoryRoot(start: string): Promise<string> {
  ...
  for (;;) {
    try {
      ...
      await access(resolve(candidate, 'infra/db/schema/0006_ingestion_runtime_history.sql'));
      return candidate;
    } catch {
      const parent = dirname(candidate);
      if (parent === candidate) throw new Error('elceo_repository_root_not_found');
      candidate = parent;
    }
  }
}
...
export async function runCanonicalCandlePostgresTests(): Promise<void> {
  ...
  const repositoryRoot = await findRepositoryRoot(__dirname);
  const schemaRoot = resolve(repositoryRoot, 'infra/db/schema');
  const migrationFiles = (await readdir(schemaRoot)).filter((name) => /^(0006|0007).*\.sql$/.test(name)).sort();
  assert(migrationFiles.length === 2, 'existing ingestion migrations 0006 and 0007 must be present');
  for (const file of migrationFiles) await pool.query(await readFile(resolve(schemaRoot, file), 'utf8'));
  ...
  try {
    ...
    const repository = new SqlIngestionEventSnapshotRepository(async <T extends Record<string, unknown>>(sql: string, params: unknown[] = []) => (await pool.query(sql, params)).rows as T[]);
    ...
    assert((await repository.getEventsByRunId(runId)).length === 1, 'SQL identical duplicate behavior must match memory semantics');
    await repository.saveEventSnapshots(runId, 'XAU/USD', 'H1', [later, first]);
    const beforeConflict = await repository.getEventsByRunId(runId);
    let conflict = false;
