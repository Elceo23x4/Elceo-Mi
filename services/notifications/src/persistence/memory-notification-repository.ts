...
export class MemoryNotificationOutboxRepository implements NotificationOutboxRepository {
  ...
  async claimDueOutboxItems(asOfIso:string,claimExpiresAt:string,limit:number,claimToken:string):Promise<NotificationOutboxRecord[]>{const due=[...this.byId.values()].filter(c=>((c.status==='staged'||c.status==='failed')&&Date.parse(c.availableAt)<=Date.parse(asOfIso))||(c.status==='dispatching'&&!!c.claimExpiresAt&&Date.parse(c.claimExpiresAt)<=Date.parse(asOfIso))).sort((a,b)=>Date.parse(a.availableAt)-Date.parse(b.availableAt)||Date.parse(a.createdAt)-Date.parse(b.createdAt)||a.outboxId.localeCompare(b.outboxId)).slice(0,limit);return due.map(c=>{const n={...c,status:'dispatching' as const,claimToken,claimGeneration:(c.claimGeneration??0)+1,claimedAt:asOfIso,claimExpiresAt,lastAttemptAt:asOfIso,updatedAt:asOfIso};this.byId.set(n.outboxId,n);this.byKey.set(n.outboxKey,n);return n;});}
  ...
  private owns(item:NotificationOutboxRecord){const c=this.byId.get(item.outboxId);return c?.claimToken===item.claimToken&&c?.claimGeneration===item.claimGeneration;}
  async markClaimDelivered(item:NotificationOutboxRecord,at:string){if(!this.owns(item))return false;await this.markDelivered(item.outboxId,at);return true;}
  async markClaimFailed(item:NotificationOutboxRecord,at:string,next:string,code:string|null,message:string|null){if(!this.owns(item))return false;await this.markFailed(item.outboxId,at,next,code,message);return true;}
  async markClaimDead(item:NotificationOutboxRecord,at:string,code:string|null,message:string|null){if(!this.owns(item))return false;await this.markDead(item.outboxId,at,code,message);return true;}
  async markClaimAmbiguous(item:NotificationOutboxRecord,at:string,code:string|null,message:string|null){if(!this.owns(item))return false;const c=this.byId.get(item.outboxId)!;const n={...c,status:'ambiguous' as const,ambiguousAt:at,attemptCount:c.attemptCount+1,lastAttemptAt:at,lastErrorCode:code,lastErrorMessage:message,claimToken:null,claimedAt:null,claimExpiresAt:null,updatedAt:at};this.byId.set(n.outboxId,n);this.byKey.set(n.outboxKey,n);return true;}
