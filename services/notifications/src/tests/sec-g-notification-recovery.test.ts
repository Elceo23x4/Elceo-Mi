...
export async function runSecGNotificationRecoveryTests(){
 ...
 if(process.env.SEC_G_ARTIFACTS==='1'){
  ...
  const root=resolve(process.env.SEC_G_ARTIFACT_DIR??resolve(process.env.INIT_CWD??process.cwd(),'artifacts/sec-g'));
  const path=resolve(root,'notification-recovery.json');
