...
export async function runVerificationProviderTests(): Promise<void> {
  ...
  const issue = await service.issueTargetVerification('email-target', '2026-01-01T01:00:00.000Z');
  ...
  const badConsume = await service.consumeTargetVerification('email-target', 'wrong', '2026-01-01T01:10:00.000Z');
  assert(badConsume.verified === false && badConsume.reason === 'invalid_verification_token', 'wrong token should fail');

  const okConsume = await service.consumeTargetVerification('email-target', 'fixed-token', '2026-01-01T01:11:00.000Z');
  ...
  const already = await service.issueTargetVerification('email-target', '2026-01-01T01:12:00.000Z');
  ...
  const pushIssue = await service.issueTargetVerification('push-target', '2026-01-01T02:00:00.000Z');
  const pushResend = await service.resendTargetVerification('push-target', '2026-01-01T02:05:00.000Z');
