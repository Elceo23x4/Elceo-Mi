...
export type MaterializationLease = { jobHash: string; ownerToken: string; generation: number; acquiredAt: number; expiresAt: number };
export type LeaseAcquireResult = { acquired: true; lease: MaterializationLease } | { acquired: false; reason: 'adaptive_scheduler_follower' | 'adaptive_scheduler_unavailable' };
...
export type MaterializationRepository = { publish<T>(lease: MaterializationLease, artifact: CanonicalArtifact<T>): Promise<boolean>; readCurrent<T>(scopeHash: string): Promise<CanonicalArtifactRead<T>>; getByIdentity<T>(identity: string): Promise<CanonicalArtifact<T> | null> };
export type AdaptiveOwnershipStore = { readonly kind: 'redis'; acquire(jobHash: string, ownerToken: string, leaseMs: number): Promise<LeaseAcquireResult>; renew(lease: MaterializationLease, leaseMs: number): Promise<MaterializationLease | null>; complete(lease: MaterializationLease, nextDueAt: number): Promise<boolean>; release(lease: MaterializationLease): Promise<boolean>; isCurrent(lease: MaterializationLease): Promise<boolean>; close(): Promise<void> };
