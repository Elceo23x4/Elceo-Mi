...
export const buildAggregateReasoningInputIdentity=(v:{evidenceIdentities:readonly string[];asset:string;horizon:string;weightingPolicyVersion:string;ruleVersions:readonly string[];evaluationEpoch:number})=>`reasoning-input:${hash({...v,evidenceIdentities:[...v.evidenceIdentities].sort(),ruleVersions:[...v.ruleVersions].sort()})}`;
export const buildCanonicalCognitionIdentity=(v:{reasoningInputIdentity:string;asset:string;horizon:string;weightingPolicyVersion:string;ruleVersions:readonly string[];evaluationEpoch:number})=>`cognition:${hash({...v,ruleVersions:[...v.ruleVersions].sort()})}`;
