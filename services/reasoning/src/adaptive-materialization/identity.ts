...
export const buildAggregateReasoningInputIdentity=(v:{evidenceIdentities:readonly string[];asset:string;horizon:string;weightingPolicyVersion:string;ruleVersions:readonly string[];freshnessPolicyVersion:string;freshnessMs:number;evaluationEpoch:number})=>`reasoning-input:${hash({...v,evidenceIdentities:[...v.evidenceIdentities].sort(),ruleVersions:[...v.ruleVersions].sort()})}`;
