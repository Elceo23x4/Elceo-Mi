...
export class ReadOptimizedImmutableMaterializationStore implements ImmutableMaterializationStore {
  ...
  private valid<T>(identity:string,artifact:CanonicalArtifact<T>|null):artifact is CanonicalArtifact<T>{if(!artifact||artifact.identity!==identity||artifact.schemaVersion!=='canonical_materialization_v1'||!['evidence','cognition','dashboard_projection'].includes(artifact.kind))return false;const{integrityHash,...body}=artifact;return integrityHash===buildArtifactIntegrityHash(body)}
