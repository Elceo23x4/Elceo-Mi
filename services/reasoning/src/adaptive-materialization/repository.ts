...
export class SqlImmutableMaterializationStore implements ImmutableMaterializationStore {
  ...
  async saveImmutable<T>(a:CanonicalArtifact<T>){const result=await this.pool.query("INSERT INTO app_canonical_materializations (identity,scope_hash,kind,artifact_json,integrity_hash,created_at) VALUES ($1,$2,$3,$4::jsonb,$5,$6) ON CONFLICT (identity) DO UPDATE SET identity=EXCLUDED.identity WHERE app_canonical_materializations.integrity_hash=EXCLUDED.integrity_hash AND app_canonical_materializations.artifact_json=EXCLUDED.artifact_json RETURNING artifact_json::text AS artifact_json",[a.identity,a.scopeHash,a.kind,JSON.stringify(a),a.integrityHash,a.generatedAt]);if(result.rows.length!==1)throw new Error(`immutable_materialization_identity_conflict:${a.identity}`)}
...
export class ReadOptimizedImmutableMaterializationStore implements ImmutableMaterializationStore {
  ...
  private valid<T>(identity:string,artifact:CanonicalArtifact<T>|null):artifact is CanonicalArtifact<T>{if(!artifact||artifact.identity!==identity||artifact.schemaVersion!=='canonical_materialization_v1'||!['evidence','cognition','dashboard_projection','kick_off_dashboard_context'].includes(artifact.kind))return false;const{integrityHash,...body}=artifact;return integrityHash===buildArtifactIntegrityHash(body)}
