...
export class MemoryMaterializationRepository implements MaterializationRepository {
  ...
  async publish<T>(lease:MaterializationLease,artifact:CanonicalArtifact<T>){if(!await this.ownership.isCurrent(lease))return false;this.immutable.set(artifact.identity,artifact);if(!await this.ownership.isCurrent(lease))return false;this.current.set(artifact.scopeHash,artifact.identity);return true}
...
export class FencedMaterializationRepository implements MaterializationRepository {
  constructor(private readonly ownership:Pick<AdaptiveOwnershipStore,'publishCurrent'|'readCurrentIdentity'>,private readonly immutable:ImmutableMaterializationStore,private readonly nextDueAt:(artifact:CanonicalArtifact<unknown>)=>number,private readonly now:()=>number=Date.now){}
  async publish<T>(lease:MaterializationLease,artifact:CanonicalArtifact<T>){await this.immutable.saveImmutable(artifact);return this.ownership.publishCurrent(lease,artifact.scopeHash,artifact.identity,this.nextDueAt(artifact))}
  async readCurrent<T>(scopeHash:string):Promise<CanonicalArtifactRead<T>>{const identity=await this.ownership.readCurrentIdentity(scopeHash);if(!identity)return{state:'unavailable',artifact:null};const artifact=await this.immutable.getImmutable<T>(identity);if(!artifact)return{state:'unavailable',artifact:null};return{state:Date.parse(artifact.freshUntil)>=this.now()?'available':'stale',artifact}}
