...
export function resolveAssetDirectionFromNormalizedMacroContext(input: MarketAssetDirectionResolutionInput & { indicatorKind: string; category?: string; currency?: string; region?: string; signedNormalizedScore: number; economicMeaning?: string | null; policyPressure?: string | null; growthPressure?: string | null; inflationPressure?: string | null; riskPressure?: string | null; macroNormalizationComplete?: boolean; evidenceConfidence?: number | null }): MarketAssetDirectionResolutionResult {
  ...
  return result(input, m, raw, direction, target, Math.min(incomplete ? 35 : 68, confidenceCeiling), codes, warnings, ['c6-r4-macro-surprise-normalized'], `C6-R4 normalized ${input.indicatorKind} as ${context.surpriseDirection}/${economicMeaning}; downstream asset direction remains context-aware and price confirmation is pending.`);
}
