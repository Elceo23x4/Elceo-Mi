...
import { MARKET_ASSET_CAUSALITY_ASSETS, MARKET_ASSET_DRIVER_KINDS, MARKET_ASSET_RAW_DIRECTION_HINTS, MARKET_REASONING_DIAGNOSTIC_ASSETS, TRADING_ASSET_COVERAGE } from '@elceo/types';
...
export function resolvePolicyToneImpact(input: MarketAssetDirectionResolutionInput): MarketAssetDirectionResolutionResult {
  ...
  const context = resolveMarketEconomicContext({ metadata: m, issuerCurrency: m.issuerCurrency, issuerInstitution: m.issuerInstitution, issuerRegion: m.issuerRegion, policyIssuerCurrency: m.policyIssuerCurrency, affectedCurrency: input.affectedCurrency ?? m.affectedCurrency, policyIssuerRegion: input.policyIssuerRegion ?? m.policyIssuerRegion, centralBank: m.centralBank, providerId: m.providerId, provider: m.provider, source: m.source, targetAsset: input.asset }); const issuerCurrency = context.issuerCurrency !== 'unknown' && context.issuerCurrency !== 'global' && context.issuerCurrency !== 'other' ? context.issuerCurrency : null; const fed = context.issuerCurrency === 'USD'; const codes: MarketAssetDirectionResolutionReasonCode[] = ['policy_tone_asset_context','causality_map_requirement']; const warnings: MarketAssetDirectionResolutionWarning[] = context.resolutionSource === 'unresolved' ? ['ambiguous_policy_issuer'] : [];
  if (context.conflictFields.length) return result(input, m, raw, 'mixed', 'unknown', 20, [...codes,'ambiguous_context'], ['ambiguous_policy_issuer'], ['policy-issuer-conflict'], 'Conflicting structured issuer context prevents policy issuer directional contribution.', 'policy_issuer_conflict');
  ...
  if (!fed) return result(input, m, raw, 'mixed', policyPressureTargetForAmbiguousIssuer(asset), 34, [...codes,'ambiguous_context'], warnings.length ? warnings : ['ambiguous_policy_issuer'], ['policy-issuer-ambiguous'], 'Policy tone needs explicit structured issuer context before applying Fed/USD-specific asset pressure.', 'policy_issuer_missing_or_unresolved');
...
export function resolveAssetDirectionFromNormalizedMacroContext(input: MarketAssetDirectionResolutionInput & { indicatorKind: string; category?: string; currency?: string; region?: string; signedNormalizedScore: number; economicMeaning?: string | null; policyPressure?: string | null; growthPressure?: string | null; inflationPressure?: string | null; riskPressure?: string | null; macroNormalizationComplete?: boolean; evidenceConfidence?: number | null; auditSurpriseDirection?: string | null }): MarketAssetDirectionResolutionResult {
  ...
  else if (!incomplete && FX.includes(asset)) { const normalizedCurrency = input.currency && !['global','unknown'].includes(input.currency) ? input.currency : null; if (normalizedCurrency === 'USD' && (policyPressure === 'dovish' || economicMeaning === 'weaker_labor')) { const orientation = resolveFxPairOrientation(asset); direction = orientation?.base === 'USD' ? 'bearish' : orientation?.quote === 'USD' ? 'bullish' : 'mixed'; target = fxTargetFromCurrency(asset, 'USD'); } else { const fxInput: Parameters<typeof resolveFxRelativeStrength>[0] = { pairAsset: asset as Parameters<typeof resolveFxRelativeStrength>[0]['pairAsset'] }; fxInput.metadataJson = input.metadataJson ?? JSON.stringify({ indicatorKind: input.indicatorKind, category: input.category, currency: input.currency, issuerCurrency: input.currency, region: input.region, policyTone: policyPressure, economicMeaning, growthPressure }); const fx = resolveFxRelativeStrength(fxInput); direction = fxDirectionFromPairDirection(fx.pairDirection); target = fxTargetFromCurrency(asset, normalizedCurrency); } }
  ...
  else if (!incomplete && (US_EQUITIES.includes(asset) || asset === 'btc_usd')) { direction = growthPressure === 'weaker' || economicMeaning === 'weaker_labor' ? 'bearish' : policyPressure === 'hawkish' ? 'mixed' : policyPressure === 'dovish' ? 'bullish' : 'mixed'; target = asset === 'btc_usd' ? 'liquidity_complex' : 'risk_complex'; }
  const confidenceCeiling = typeof input.evidenceConfidence === 'number' && Number.isFinite(input.evidenceConfidence) ? input.evidenceConfidence : 68;
  return result(input, m, raw, direction, target, Math.min(incomplete ? 35 : 68, confidenceCeiling), codes, warnings, ['c6-r4-macro-surprise-normalized'], `C6-R4 normalized ${input.indicatorKind} as ${input.auditSurpriseDirection ?? context.surpriseDirection}/${economicMeaning}; downstream asset direction remains context-aware and price confirmation is pending.`);
}
...
function macroDirection(input: MarketAssetDirectionResolutionInput, m: Metadata, _raw: MarketAssetRawDirectionHint, _asset: MarketAssetCausalityAsset): MarketAssetDirectionResolutionResult | null {
  ...
  return resolveAssetDirectionFromNormalizedMacroContext({ ...input, indicatorKind: normalized.indicatorKind, category: normalized.category, currency: normalized.currency, region: normalized.region, signedNormalizedScore: normalized.normalizedSurpriseScore, economicMeaning: normalized.economicMeaning, policyPressure: normalized.policyPressure, growthPressure: normalized.growthPressure, inflationPressure: normalized.inflationPressure, riskPressure: normalized.riskPressure, macroNormalizationComplete: !incomplete, evidenceConfidence: normalized.confidence, auditSurpriseDirection: normalized.surpriseDirection });
}
...
export function resolveAssetContextualEvidenceDirection(input: MarketAssetDirectionResolutionInput): MarketAssetDirectionResolutionResult {
  ...
  if ((driver === 'crypto_etf_flows' || driver === 'crypto_onchain') && asset === 'btc_usd') return result(input, m, raw, raw === 'negative' || raw === 'bearish' ? 'bearish' : 'bullish', 'crypto_native', 74, ['crypto_native_driver'], ['requires_price_confirmation'], ['crypto-native-btc'], 'Crypto ETF/on-chain evidence is resolved only for BTC native context.');
  if (driver === 'dollar_liquidity' && asset === 'dxy') return result(input, m, raw, raw === 'bearish' || raw === 'negative' ? 'bearish' : 'bullish', 'usd_side', 62, ['usd_side_policy_pressure','causality_map_requirement'], ['requires_price_confirmation','pending_fx_relative_strength'], ['dxy-dollar-liquidity'], 'Dollar-liquidity evidence is diagnostic USD-side context for DXY.');
  if (driver === 'volatility_surface' && asset === 'vix') return result(input, m, raw, raw === 'bearish' || raw === 'negative' ? 'bearish' : 'bullish', 'volatility_complex', 64, ['risk_regime_asset_context','causality_map_requirement'], ['requires_price_confirmation'], ['vix-volatility-surface'], 'Volatility-surface evidence maps directly to VIX diagnostic pressure.');
  if (driver === 'safe_haven_demand') {
