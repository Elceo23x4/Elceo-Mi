import type { MarketCognitionSnapshot, MarketConfidenceCalibrationAsset, MarketConfidenceCalibrationBoost, MarketConfidenceCalibrationComponent, MarketConfidenceCalibrationCoverageReport, MarketConfidenceCalibrationInput, MarketConfidenceCalibrationPenalty, MarketConfidenceCalibrationPenaltyKind, MarketConfidenceCalibrationReasonCode, MarketConfidenceCalibrationResult, MarketConfidenceCalibrationRule, MarketConfidenceCalibrationRuleSetSnapshot, MarketConfidenceCalibrationWarning, MarketContradictionMatrixResult, WeightedEvidenceSnapshot } from '@elceo/types';
...
export function calibrateMarketConfidence(input: MarketConfidenceCalibrationInput): MarketConfidenceCalibrationResult {
  ...
  if (hasWarning(input, 'missing_provider_reliability') || input.options?.providerReliabilitySupplied !== true) { addPenalty(penalties, 'missing_provider_reliability', 8, false, 'Provider reliability weighting remains pending and caps the highest confidence tier.'); reasonCodes.push('provider_readiness_gap'); warnings.push('provider_readiness_penalty_applied'); }
...
export function buildConfidenceCalibrationInputFromWeightedSnapshot(weightedSnapshot: WeightedEvidenceSnapshot, options?: MarketConfidenceCalibrationInput['options']): MarketConfidenceCalibrationInput {
  ...
  const contradictionMatrix = evaluateContradictionsFromWeightedSnapshot(weightedSnapshot, options);
  return { asset:weightedSnapshot.asset, horizon:weightedSnapshot.horizon, generatedAt:weightedSnapshot.generatedAt, baseConfidence, evidenceQuality:clamp(evidenceQuality), usableWeight:clamp(usableWeight), freshness, coverage, weightedSnapshot, contradictionMatrix, warnings:[...weightedSnapshot.warnings, ...fxWarnings, ...contradictionMatrix.warnings], reasonCodes:[...contradictionMatrix.reasonCodes], ...(options ? { options } : {}) };
}
export function calibrateConfidenceFromWeightedSnapshot(weightedSnapshot: WeightedEvidenceSnapshot, options?: MarketConfidenceCalibrationInput['options']): MarketConfidenceCalibrationResult { return calibrateMarketConfidence(buildConfidenceCalibrationInputFromWeightedSnapshot(weightedSnapshot, options)); }
