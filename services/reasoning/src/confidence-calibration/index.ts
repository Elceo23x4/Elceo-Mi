...
export function calibrateMarketConfidence(input: MarketConfidenceCalibrationInput): MarketConfidenceCalibrationResult {
  ...
  const suppliedReaction = input.options?.priceReaction;
  if (suppliedReaction?.status === 'rejected' || suppliedReaction?.status === 'reversed') { addPenalty(penalties, 'high_contradiction_severity', 12, true, 'Rejected or reversed event-window price reaction lowers confidence as contradiction context.'); reasonCodes.push('contradiction_severity_penalty'); warnings.push('contradiction_penalty_applied'); }
  if (suppliedReaction?.status === 'absorbed' || suppliedReaction?.status === 'ambiguous' || suppliedReaction?.status === 'delayed') { addPenalty(penalties, 'missing_price_confirmation', 6, false, 'Absorbed, ambiguous, or delayed price reaction keeps confidence cautious.'); reasonCodes.push('pending_confirmation_penalty'); warnings.push('price_confirmation_penalty_applied'); }
  const priceConfirmed = suppliedReaction?.status === 'confirmed';
  ...
  if (eventSensitive && !priceConfirmed) { addPenalty(penalties, 'missing_price_confirmation', 14, true, 'Event-sensitive evidence is capped until deterministic price reaction confirmation is available.'); reasonCodes.push('price_confirmation_pending'); warnings.push('price_confirmation_penalty_applied'); }
...
export function getMarketConfidenceCalibrationRuleSetSnapshot(asOfIso = new Date().toISOString()): MarketConfidenceCalibrationRuleSetSnapshot { const snapshot = { generatedAt:asOfIso, rules, warnings:[...MARKET_CONFIDENCE_CALIBRATION_WARNINGS], readiness:getMarketReasoningModuleReadiness('confidence_calibration') }; const v=validateMarketConfidenceCalibrationRuleSetSnapshot(snapshot); if(v.ok===false) throw new Error(`invalid_market_confidence_calibration_rules:${v.errors.join('|')}`); return snapshot; }
export function getMarketConfidenceCalibrationCoverageReport(asOfIso = new Date().toISOString()): MarketConfidenceCalibrationCoverageReport { const report = { generatedAt:asOfIso, componentKinds:[...MARKET_CONFIDENCE_CALIBRATION_COMPONENT_KINDS], penaltyKinds:[...MARKET_CONFIDENCE_CALIBRATION_PENALTY_KINDS], boostKinds:[...MARKET_CONFIDENCE_CALIBRATION_BOOST_KINDS], warnings:[...MARKET_CONFIDENCE_CALIBRATION_WARNINGS], notes:['C6-R6 confidence calibration is deterministic foundation-level market readiness scoring.','C6-R7 deterministic price reaction is available when supplied; provider reliability weighting, golden scenario expansion, and empirical backtesting remain pending.'], readiness:getMarketReasoningModuleReadiness('confidence_calibration') }; const v=validateMarketConfidenceCalibrationCoverageReport(report); if(v.ok===false) throw new Error(`invalid_market_confidence_calibration_coverage:${v.errors.join('|')}`); return report; }
