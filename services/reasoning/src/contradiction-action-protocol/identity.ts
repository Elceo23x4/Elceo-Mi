...
function normalizedInput(bundle: ProtocolEvidenceBundle) {
  return { ...bundle.contradictionInput, warnings: strings(bundle.contradictionInput.warnings), evidencePoints: byCanonicalContent(bundle.contradictionInput.evidencePoints.map((point) => ({ ...point, warnings: strings(point.warnings), reasonCodes: strings(point.reasonCodes) }))) };
}

export function protocolEvidenceSnapshotId(bundle: ProtocolEvidenceBundle): string {
  return `cap-evidence:${canonicalHash({ eventEvaluationId: bundle.eventEvaluation.eventEvaluationId, assessmentEvidenceHash: bundle.eventEvaluation.assessmentEvidenceHash, expectationId: bundle.expectation.expectationId, contradictionInputHash: canonicalHash(normalizedInput(bundle)), contradictionHash: canonicalHash(normalizedContradiction(bundle.contradictionMatrix)), invalidationHash: canonicalHash(bundle.invalidationState), cognitionSnapshotIds: strings([bundle.preEventCognition.persisted.snapshotId, ...(bundle.postEventCognition ? [bundle.postEventCognition.persisted.snapshotId] : [])]), analogRetrievalId: bundle.analogRetrieval?.retrievalId ?? null, analogQueryFeatureHash: bundle.analogRetrieval?.queryFeatureHash ?? null })}`;
}
