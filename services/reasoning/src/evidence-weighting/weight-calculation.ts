...
function toCausalityAsset(asset: TradingAssetCoverage): MarketAssetCausalityAsset { return asset; }

export function inferWeightedEvidenceDirection(item: ReasoningEvidenceInputItem, asset?: TradingAssetCoverage): WeightedEvidenceDirection {
...
export function buildWeightedEvidenceItem(inputItem:ReasoningEvidenceInputItem,asset:TradingAssetCoverage,horizon:EvidenceWeightHorizon):WeightedEvidenceItem{
  const p=getWeightPolicy(asset,inputItem.evidenceClass,horizon);
  const q=inputItem.qualityScore.finalQualityScore;
  const aw=(p.baseWeight*q)/100;
