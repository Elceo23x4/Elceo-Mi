...
export function validateObservationCandles(expectation: ExpectationRecord, observations: ObservationSet, evaluatedAt: string): string[] {
  ...
  if (Date.parse(observations.observedWindow.start) >= Date.parse(observations.observedWindow.end)) pushUnique(errors, 'invalid_observation_window');
  let lastClosed = '';
  ...
  for (const candle of observations.candles) {
    ...
    if ([candle.open, candle.high, candle.low, candle.close].some((v) => v <= 0)) pushUnique(errors, 'invalid_ohlc_rejected');
...
export function evaluateExpectationReality(params: { expectation: ExpectationRecord; observations: ObservationSet; horizon: ExpectationHorizon; evaluatedAt: string }): ExpectationRealityEvaluation {
  ...
  const candles = horizonCandles(expectation, observations, requiredBars);
  ...
  for (const c of candles) {
    ...
    if (!upConfirmAt && upVu >= EXPECTATION_REALITY_POLICY_V1.movementBoundaries.confirmationVolUnits) upConfirmAt = c.closedAt;
    if (!downConfirmAt && downVu >= EXPECTATION_REALITY_POLICY_V1.movementBoundaries.confirmationVolUnits) downConfirmAt = c.closedAt;
  ...
  else if (expectedAt) { outcome = horizon === 'follow_through' && !upConfirmAt && !downConfirmAt ? 'delayed_confirmation' : 'confirmed'; path = firstMaterialMoveDirection === (bullish ? 'down' : 'up') ? 'adverse_then_confirmed' : 'clean_confirmation'; pushUnique(reasonCodes, 'expected_direction_confirmed'); pushUnique(reasonCodes, path === 'adverse_then_confirmed' ? 'adversity_before_confirmation' : 'confirmation_before_adversity'); }
