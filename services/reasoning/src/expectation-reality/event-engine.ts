...
function resolveReleaseDirection(expectation: EventExpectationRecord, asset: string, normalized: EventRealityRecord['normalizedSurprise'], revisionAdjusted?: ReturnType<typeof revisionMeasures>): MarketAssetDirectionResolutionResult {
  ...
  const resolved = resolveAssetContextualEvidenceDirection({ asset: asset as never, evidenceClass: 'macro_release', metadataJson: releaseMetadata(expectation, normalized, revisionAdjusted), policyTone: (revisionAdjusted?.adjustedPolicyPressure ?? normalized?.policyPressure ?? 'unknown') as never, policyIssuerRegion: String(expectation.region), affectedCurrency: String(expectation.currency), observedAt: normalized ? expectation.scheduledReleaseTime : null });
...
function buildTimeline(envelope: ReactionObservationEnvelope, expectation: EventExpectationRecord, releaseObservedAt: string, direction: MarketPriceReactionExpectedDirection) {
  ...
  return { policyVersion: EXPECTATION_REALITY_POLICY_V1.eventInterpretation.timeline.version, primaryObservation: envelope, immediate, confirmation, followThrough, laterAcceptance, initialState: immediate.state, confirmationWindowState: confirmation.state, followThroughState: followThrough.state, laterAcceptanceState: laterAcceptance.state, reversalAbsorptionState: fullReaction.status === 'absorbed' ? 'absorbed' as const : fullReaction.status === 'reversed' ? 'reversed' as const : 'none' as const, acceptanceTimestamp, acceptancePhase, firstContradictionTimestamp, reversalTimestamp, initialRejectionThenAcceptance: (immediate.state === 'rejected' || fullReaction.status === 'absorbed') && laterAcceptance.state === 'confirmed' && followThrough.state === 'confirmed' && acceptancePhase === 'follow_through', initialConfirmationThenReversal: immediate.state === 'confirmed' && !!reversalTimestamp };
}
...
function revisionMeasures(expectation: EventExpectationRecord, normalized: EventRealityRecord['normalizedSurprise']) { const family=revisionFamily(String(expectation.indicatorKind), String(expectation.indicatorCategory)); if (!normalized || normalized.actual === null || normalized.forecast === null) return { revisionPolicyVersion:'event-revision-v1' as const, revisionFamily:family, rawSurpriseDelta:null, revisionDelta:null, revisionAdjustedDelta:null, revisionAdjustedDirection:'unknown' as const, adjustedSurpriseScore:null, adjustedEconomicMeaning:null, adjustedPolicyPressure:null, adjustedGrowthPressure:null, adjustedInflationPressure:null, adjustedRiskPressure:null, revisionAdjustedMateriality:'unavailable' as const }; const raw = normalized.actual - normalized.forecast; const rev = normalized.revisedPrevious !== null && normalized.previous !== null ? normalized.revisedPrevious - normalized.previous : null; const additive = family === 'inflation' || family === 'labour_strength' || family === 'inverted_labour' || family === 'growth_activity'; const orientation = family === 'inverted_labour' ? -1 : 1; const adjusted = additive && rev !== null ? (raw + rev) * orientation : raw * orientation; const direction = Math.abs(adjusted) < 0.000001 ? 'inline' as const : adjusted > 0 ? 'economic_positive' as const : 'economic_negative' as const; const materiality = !additive && rev !== null ? 'unavailable' as const : Math.abs(adjusted) >= Math.max(0.01, Math.abs(raw * orientation) * 0.5) ? 'material' as const : 'immaterial' as const; return { revisionPolicyVersion:'event-revision-v1' as const, revisionFamily:family, rawSurpriseDelta:raw, revisionDelta:rev, revisionAdjustedDelta:additive ? adjusted : null, revisionAdjustedDirection:direction, adjustedSurpriseScore: materiality === 'unavailable' ? null : Math.abs(adjusted) * 100, adjustedEconomicMeaning: materiality === 'unavailable' ? normalized.economicMeaning : direction === 'inline' ? normalized.economicMeaning : direction === 'economic_positive' ? normalized.economicMeaning : 'revision_reversed_interpretation', adjustedPolicyPressure: materiality === 'unavailable' ? normalized.policyPressure : direction === 'inline' ? normalized.policyPressure : direction === 'economic_positive' ? normalized.policyPressure : 'revision_offsets_policy_pressure', adjustedGrowthPressure: normalized.growthPressure ?? null, adjustedInflationPressure: normalized.inflationPressure ?? null, adjustedRiskPressure: normalized.riskPressure ?? null, revisionAdjustedMateriality:materiality }; }
