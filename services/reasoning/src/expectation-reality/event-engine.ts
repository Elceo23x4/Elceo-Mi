...
import { getMacroSurpriseRuleSetSnapshot, normalizeMacroSurprise, resolveMacroPressuresFromSignedNormalizedScore } from '../macro-surprise-normalization/index';
...
function postCandles(input: MarketPriceReactionInput) { const event = Date.parse(input.eventTime ?? ''); return [...input.candles].map((c)=>({ ...c, timestamp: (c as { closedAt?: string }).closedAt ?? c.timestamp })).sort((a,b)=>Date.parse(((a as { openedAt?: string }).openedAt ?? a.timestamp))-Date.parse(((b as { openedAt?: string }).openedAt ?? b.timestamp))).filter((c)=>{ const open=Date.parse((c as { openedAt?: string }).openedAt ?? c.timestamp); const close=Date.parse((c as { closedAt?: string }).closedAt ?? c.timestamp); const split=(c as { verifiedPostEventSplit?: boolean; splitProvenance?: string | null }).verifiedPostEventSplit && !!(c as { splitProvenance?: string | null }).splitProvenance; return !Number.isFinite(event) || open >= event || (split && close > event); }); }
...
function alignRelease(expectation: EventExpectationRecord, normalized: EventRealityRecord['normalizedSurprise'], directions: EventRealityRecord['actualAssetDirections'], revisionAdjusted: ReturnType<typeof revisionMeasures>): ReleaseAlignment {
  ...
  const revisionAlignment = revisionEffect === 'reverses_interpretation' ? 'contradicted' : revisionEffect === 'unavailable' ? 'insufficient_data' : revisionEffect === 'offsets_surprise' && revisionAdjusted.revisionAdjustedMateriality !== 'material' ? 'inline' : 'aligned';
...
export function interpretEventReality(params: { expectation: EventExpectationRecord; reality: EventRealityRecord; interpretedAt: string }): EventRealityEvaluation {
  ...
  else if (reality.priceReactionTimeline.initialRejectionThenAcceptance && relatedSupports(reality) && reliable(reality) && adjustedMaterial(reality) && reality.revisionAdjustedMeasures.revisionAdjustedMateriality === 'material' && Math.abs(reality.revisionAdjustedMeasures.adjustedSurpriseScore ?? 0) >= EXPECTATION_REALITY_POLICY_V1.eventInterpretation.materialSurpriseScore && (reality.releaseAlignment.status === 'contradicted' || reality.releaseAlignment.status === 'aligned') && !criticalAmbiguity) { outcome = 'mispriced_candidate'; reasonCodes.push(reality.releaseAlignment.status === 'aligned' ? 'expectation_aligned_market_initially_rejected' : 'expectation_contradicted_market_initially_rejected_actual_reality'); }
