...
export function interpretEventReality(params: { expectation: EventExpectationRecord; reality: EventRealityRecord; interpretedAt: string }): EventRealityEvaluation {
  ...
  const eventEvaluationId = finalizationStatus === 'final' ? `event-reality-${expectation.expectationId}-${reality.releaseVersion}-final` : `event-assessment-${expectation.expectationId}-${reality.releaseVersion}-${reality.observationContentHash}`;
