...
function stateFromMove(movePct: number, vol: number, direction: MarketPriceReactionExpectedDirection) { const signed = movePct * signFor(direction); const threshold = vol * 0.35; if (signed >= threshold) return 'confirmed' as const; if (signed <= -threshold) return 'rejected' as const; return 'ambiguous' as const; }
function syntheticReaction(input: MarketPriceReactionInput, base: ReturnType<typeof evaluatePriceReaction>, status: typeof base.status, movePct: number, kind: 'immediate'|'confirmation'|'follow_through') { return { ...base, status, immediateMovePct: kind === 'immediate' ? movePct : base.immediateMovePct, followThroughMovePct: kind === 'follow_through' ? movePct : base.followThroughMovePct, reasonCodes: [...new Set([...base.reasonCodes, kind === 'follow_through' ? 'follow_through_rule' : 'confirmation_rule'])] as typeof base.reasonCodes, rationale: `${base.rationale} Timeline ${kind} state preserves the original event price origin.` }; }
function buildTimeline(envelope: ReactionObservationEnvelope, expectation: EventExpectationRecord, releaseObservedAt: string, direction: MarketPriceReactionExpectedDirection) {
  ...
  const post = postCandles(canonical).slice(0,6); const first = post[0]; const origin = first?.open ?? null; const vol = canonical.volatilityBasisPct && canonical.volatilityBasisPct > 0 ? canonical.volatilityBasisPct : 0.5;
  const moveTo = (idx: number) => origin && post[idx] ? (post[idx].close - origin) / origin * 100 : 0;
  const initialState = (first ? stateFromMove(moveTo(0), vol, direction) : 'insufficient_data') as EventRealityRecord['priceReactionTimeline']['initialState'];
  const confirmationWindowState = (post.length >= 3 ? stateFromMove(moveTo(2), vol, direction) : 'insufficient_data') as EventRealityRecord['priceReactionTimeline']['confirmationWindowState'];
  const followThroughState = (post.length >= 6 ? stateFromMove(moveTo(5), vol, direction) : stateFromMove(moveTo(post.length - 1), vol, direction)) as EventRealityRecord['priceReactionTimeline']['followThroughState'];
  ...
  post.forEach((c,i)=>{ const st=stateFromMove(moveTo(i), vol, direction); if (!acceptanceTimestamp && st === 'confirmed') { acceptanceTimestamp=c.timestamp; acceptancePhase=i===0?'immediate':i<=2?'confirmation':'follow_through'; } if (!firstContradictionTimestamp && st === 'rejected') firstContradictionTimestamp=c.timestamp; });
  const reversalTimestamp = initialState === 'confirmed' && firstContradictionTimestamp ? firstContradictionTimestamp : null;
  const immediateReaction = syntheticReaction(canonical, fullReaction, initialState, moveTo(0), 'immediate');
  const confirmationWindowReaction = syntheticReaction(canonical, fullReaction, confirmationWindowState, moveTo(Math.min(2, post.length-1)), 'confirmation');
  const followThroughReaction = syntheticReaction(canonical, fullReaction, followThroughState, moveTo(Math.min(5, post.length-1)), 'follow_through');
  return { primaryObservation: envelope, immediateReaction, confirmationWindowReaction, followThroughReaction, initialState, confirmationWindowState, followThroughState, laterAcceptanceState: acceptanceTimestamp ? 'confirmed' as const : 'ambiguous' as const, reversalAbsorptionState: fullReaction.status === 'absorbed' ? 'absorbed' as const : fullReaction.status === 'reversed' ? 'reversed' as const : 'none' as const, acceptanceTimestamp, acceptancePhase, firstContradictionTimestamp, reversalTimestamp, initialRejectionThenAcceptance: (initialState === 'rejected' || fullReaction.status === 'absorbed') && !!acceptanceTimestamp && acceptancePhase !== 'immediate', initialConfirmationThenReversal: initialState === 'confirmed' && !!reversalTimestamp };
}
function revisionFamily(indicatorKind: string, category: string) { const key=`${indicatorKind} ${category}`.toLowerCase(); if (key.includes('unemployment') || key.includes('jobless')) return 'inverted_labour' as const; if (key.includes('payroll') || key.includes('earnings') || key.includes('labour') || key.includes('labor')) return 'labour_strength' as const; if (key.includes('inflation') || key.includes('cpi') || key.includes('pce')) return 'inflation' as const; if (key.includes('gdp') || key.includes('retail') || key.includes('pmi') || key.includes('growth')) return 'growth_activity' as const; if (key.includes('central') || key.includes('policy') || key.includes('rate')) return 'central_bank_policy' as const; return 'non_additive' as const; }
function revisionMeasures(expectation: EventExpectationRecord, normalized: EventRealityRecord['normalizedSurprise']) { const family=revisionFamily(String(expectation.indicatorKind), String(expectation.indicatorCategory)); if (!normalized || normalized.actual === null || normalized.forecast === null) return { revisionPolicyVersion:'event-revision-v1' as const, revisionFamily:family, rawSurpriseDelta:null, revisionDelta:null, revisionAdjustedDelta:null, revisionAdjustedDirection:'unknown' as const, adjustedEconomicMeaning:null, adjustedPolicyPressure:null, revisionAdjustedMateriality:'unavailable' as const }; const raw = normalized.actual - normalized.forecast; const rev = normalized.revisedPrevious !== null && normalized.previous !== null ? normalized.revisedPrevious - normalized.previous : null; const additive = family === 'inflation' || family === 'labour_strength' || family === 'inverted_labour' || family === 'growth_activity'; const orientation = family === 'inverted_labour' ? -1 : 1; const adjusted = additive && rev !== null ? (raw + rev) * orientation : raw * orientation; const direction = Math.abs(adjusted) < 0.000001 ? 'inline' as const : adjusted > 0 ? 'economic_positive' as const : 'economic_negative' as const; const materiality = !additive && rev !== null ? 'unavailable' as const : Math.abs(adjusted) >= Math.max(0.01, Math.abs(raw * orientation) * 0.5) ? 'material' as const : 'immaterial' as const; return { revisionPolicyVersion:'event-revision-v1' as const, revisionFamily:family, rawSurpriseDelta:raw, revisionDelta:rev, revisionAdjustedDelta:additive ? adjusted : null, revisionAdjustedDirection:direction, adjustedEconomicMeaning: materiality === 'unavailable' ? normalized.economicMeaning : direction === 'inline' ? normalized.economicMeaning : direction === 'economic_positive' ? normalized.economicMeaning : 'revision_reversed_interpretation', adjustedPolicyPressure: materiality === 'unavailable' ? normalized.policyPressure : direction === 'inline' ? normalized.policyPressure : direction === 'economic_positive' ? normalized.policyPressure : 'revision_offsets_policy_pressure', revisionAdjustedMateriality:materiality }; }
...
function alignRelease(expectation: EventExpectationRecord, normalized: EventRealityRecord['normalizedSurprise'], directions: EventRealityRecord['actualAssetDirections'], revisionAdjusted: ReturnType<typeof revisionMeasures>): ReleaseAlignment {
  ...
  return { status, reasonCodes, economicMeaningAlignment: meaning, policyPressureAlignment: pressure, actualVsForecastAlignment: actualVsForecast, revisionEffect, primaryAssetDirectionAlignment: primaryDir, relatedMarketDirectionAlignment: relatedAlignment, expectedEconomicMeaning: expectation.expectedEconomicMeaning, actualEconomicMeaning: revisionAdjusted.adjustedEconomicMeaning ?? normalized.economicMeaning, expectedPolicyPressure: expectation.expectedPolicyPressure, actualPolicyPressure: revisionAdjusted.adjustedPolicyPressure ?? normalized.policyPressure, expectedDirection: expectation.expectedAssetDirection, actualDirection: primaryDirection as never };
}
