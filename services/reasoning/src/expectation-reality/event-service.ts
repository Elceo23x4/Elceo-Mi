...
export class EventExpectationRealityService {
  ...
  private async verifyEnvelope(envelope: ReactionObservationEnvelope, interpretedAt: string, releaseObservedAt: string): Promise<ReactionObservationEnvelope> { validateEnvelopeCutoff(envelope, interpretedAt, releaseObservedAt); const calculatedContentHash = calculateReactionEnvelopeContentHash(envelope); if (envelope.suppliedContentHash && envelope.suppliedContentHash !== calculatedContentHash) throw new Error('reaction_observation_content_hash_mismatch'); const verified = await this.verifier(envelope); if (!verified.verificationRef.trim()) throw new Error('reaction_verification_ref_missing'); const verifiedAt = parseIso(verified.verifiedAt, 'reaction_verifier_verified_at'); const availability = Math.max(...envelope.reactionInput.candles.map((c)=>parseIso(c.closedAt, 'reaction_candle_closed_at'))); if (verifiedAt < availability) throw new Error('reaction_verification_before_data_available'); if (verifiedAt > parseIso(interpretedAt, 'interpreted_at')) throw new Error('future_reaction_verification_rejected'); for (const c of envelope.reactionInput.candles.filter((x)=>x.verifiedPostEventSplit)) { const match = (verified.splitVerifications ?? []).find((v)=>v.parentCandleRef===c.parentCandleRef && v.splitAt===c.splitAt); if (!match) throw new Error('reaction_split_not_verified'); if (!match.splitVerificationRef.trim()) throw new Error('reaction_split_verification_ref_missing'); const splitVerifiedAt=parseIso(match.verifiedAt, 'reaction_split_verified_at'); if (splitVerifiedAt > parseIso(interpretedAt, 'interpreted_at')) throw new Error('future_reaction_split_verification_rejected'); } return { ...envelope, calculatedContentHash, effectiveReliability: verified.effectiveReliability, trustBasis: verified.trustBasis, verificationRef: verified.verificationRef, verifiedAt: verified.verifiedAt, splitVerifications: verified.splitVerifications ?? [] };  }

  async evaluateEvent(params: { expectationId: string; release: NumericReleaseFields | { releaseId: string; nonNumericOutcome: string; observedAt: string; releaseVersion: string; provenance: EventExpectationRecord['provenance'] }; primaryObservationInput: ReactionObservationEnvelope; relatedMarketObservationInputs: ReactionObservationEnvelope[]; postEventCognitionSnapshotId?: string | null; interpretedAt: string; relatedEvidenceDecision?: import('./contracts').RelatedEvidenceDecision }): Promise<EventRealityEvaluation> {
    ...
    const reality = buildEventReality({ expectation, release, primaryPriceReactionInput: primary, relatedMarketReactionInputs: related, postEventCognition, ...(params.relatedEvidenceDecision ? { relatedEvidenceDecision: params.relatedEvidenceDecision } : {}) });
