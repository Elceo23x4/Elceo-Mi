...
export class EventExpectationRealityService {
  ...
  private resolveRelatedEvidenceDecision(expectation: EventExpectationRecord, related: ReactionObservationEnvelope[], reality: ReturnType<typeof buildEventReality>, releaseObservedAt: string, interpretedAt: string, closure?: { finalEvidenceDecisionAt?: string | null; structurallyUnavailableAssets?: string[]; availabilityVerificationRef?: string | null; closureDeadlineMinutes?: number | null }): RelatedEvidenceDecision {
    const required = [...new Set(expectation.requiredRelatedAssets ?? [])];
    const supplied = new Set(related.map((r)=>r.reactionInput.asset as EventExpectationRecord['asset']));
    ...
    const decisionAt = closure?.finalEvidenceDecisionAt ?? interpretedAt;
    const assertDecisionTime = () => { const t=parseIso(decisionAt, 'related_evidence_decision_at'); if (t < release || t > interpreted) throw new Error('related_evidence_decision_time_invalid'); return decisionAt; };
    if (required.length === 0) return { status:'explicitly_unavailable', decidedAt: assertDecisionTime(), policyVersion:'related-evidence-closure-v1', reasonCodes:['event_contract_related_confirmation_not_required'] };
    if (missing.length === 0 && reality.relatedMarketState === 'confirmed') return { status:'confirmed', decidedAt: assertDecisionTime(), policyVersion:'related-evidence-closure-v1', reasonCodes:['required_related_evidence_confirmed'] };
    if (missing.length === 0 && reality.relatedMarketState === 'conflicting') return { status:'conflicting_final', decidedAt: assertDecisionTime(), policyVersion:'related-evidence-closure-v1', reasonCodes:['required_related_evidence_conflicting'] };
    if (missing.length > 0 && closure?.structurallyUnavailableAssets && closure.availabilityVerificationRef && missing.every((a)=>closure.structurallyUnavailableAssets!.includes(a))) return { status:'explicitly_unavailable', decidedAt: assertDecisionTime(), policyVersion:'related-evidence-closure-v1', reasonCodes:['required_related_evidence_structurally_unavailable'] };
    const deadlineElapsed = typeof closure?.closureDeadlineMinutes === 'number' && interpreted >= release + closure.closureDeadlineMinutes * 60_000;
    if (closure?.finalEvidenceDecisionAt || deadlineElapsed) return { status:'insufficient_final', decidedAt: assertDecisionTime(), policyVersion:'related-evidence-closure-v1', reasonCodes:['required_related_evidence_insufficient_final'] };
    return { status:'pending', decidedAt:null, policyVersion:'related-evidence-closure-v1', reasonCodes:['required_related_evidence_pending'] };
  }

  async evaluateEvent(params: { expectationId: string; release: NumericReleaseFields | { releaseId: string; nonNumericOutcome: string; observedAt: string; releaseVersion: string; provenance: EventExpectationRecord['provenance'] }; primaryObservationInput: ReactionObservationEnvelope; relatedMarketObservationInputs: ReactionObservationEnvelope[]; postEventCognitionSnapshotId?: string | null; interpretedAt: string; relatedEvidenceClosure?: { finalEvidenceDecisionAt?: string | null; structurallyUnavailableAssets?: string[]; availabilityVerificationRef?: string | null; closureDeadlineMinutes?: number | null } }): Promise<EventRealityEvaluation> {
    ...
    const relatedEvidenceDecision = this.resolveRelatedEvidenceDecision(expectation, related, preliminary, release.observedAt, params.interpretedAt, params.relatedEvidenceClosure);
