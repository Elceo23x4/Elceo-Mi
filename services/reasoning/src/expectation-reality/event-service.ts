...
const isTrusted = (envelope: ReactionObservationEnvelope) => (EXPECTATION_REALITY_POLICY_V1.eventInterpretation.reliableProvenance as readonly string[]).includes(envelope.effectiveReliability ?? 'unverified');
...
export class EventExpectationRealityService {
  ...
  private async resolveRelatedEvidenceDecision(expectation: EventExpectationRecord, related: ReactionObservationEnvelope[], reality: ReturnType<typeof buildEventReality>, releaseObservedAt: string, interpretedAt: string): Promise<RelatedEvidenceDecision> {
    ...
    if (unresolvedRequired.length > 0) {
      const availability = await Promise.all([...new Set(unresolvedRequired)].map((asset)=>this.availabilityVerifier({ asset, expectation, releaseObservedAt, interpretedAt })));
      const unavailable = availability.filter((a)=>a.availability === 'unavailable');
      for (const a of unavailable) { if (a.asset !== unresolvedRequired.find((x)=>x===a.asset)) throw new Error('related_availability_asset_mismatch'); if (!a.verificationRef.trim()) throw new Error('related_availability_verification_ref_missing'); if (!a.trustBasis.trim()) throw new Error('related_availability_trust_basis_missing'); const t=parseIso(a.verifiedAt, 'related_availability_verified_at'); if (t < release || t > interpreted) throw new Error('related_availability_verification_time_invalid'); }
      if (unavailable.length === new Set(unresolvedRequired).size) return withPolicy({ status:'explicitly_unavailable', decidedAt:decided(), reasonCodes:['required_related_evidence_structurally_unavailable'], evidenceRefs: unavailable.map((a)=>`${a.asset}:${a.verificationRef}`), availabilityEvidence: unavailable.map((a)=>({asset:a.asset, availability:a.availability, verificationRef:a.verificationRef, verifiedAt:a.verifiedAt, trustBasis:a.trustBasis})) });
    }
  ...
  async evaluateEvent(params: { expectationId: string; release: NumericReleaseFields | { releaseId: string; nonNumericOutcome: string; observedAt: string; releaseVersion: string; provenance: EventExpectationRecord['provenance'] }; primaryObservationInput: ReactionObservationEnvelope; relatedMarketObservationInputs: ReactionObservationEnvelope[]; postEventCognitionSnapshotId?: string | null; interpretedAt: string; relatedEvidenceClosure?: never }): Promise<EventRealityEvaluation> {
    ...
    const relatedAssets = related.map((r)=>r.reactionInput.asset);
    if (new Set(relatedAssets).size !== relatedAssets.length) throw new Error('duplicate_related_market_observation_rejected');
