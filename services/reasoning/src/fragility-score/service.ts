...
export class FragilityScoreService{
 ...
 async evaluate(input:FragilityScoreServiceInput):Promise<FragilityScoreEvaluation>{
  ...
  same(positioning.sourceCleanlinessEvaluationId,cleanliness.cleanlinessEvaluationId,'fragility_positioning_cleanliness_lineage_mismatch');
  same(positioning.sourceNarrativeDecayEvaluationId,narrative.narrativeDecayEvaluationId,'fragility_positioning_narrative_lineage_mismatch');
  ...
  for(const reference of cleanliness.components.flatMap(component=>component.sourceReferences))before(reference.availableAt,input.evidenceCutoffAt,'fragility_cleanliness_evidence_after_cutoff');
