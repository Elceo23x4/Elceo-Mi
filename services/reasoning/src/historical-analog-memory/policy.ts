...
export type HistoricalAnalogStructuralAvailabilityRule = { asset?: string; indicatorKind?: string; indicatorCategory?: string; queryStage?: string; structurallyUnavailable: true; reason: string };
...
export function resolveHistoricalAnalogStructuralAvailability(input: { asset: string; indicatorKind: string; indicatorCategory: string; queryStage: string }, rules: readonly HistoricalAnalogStructuralAvailabilityRule[] = HISTORICAL_ANALOG_COVERAGE_STRUCTURAL_RULES): { structurallyUnavailable: boolean; reason: string | null; policyVersion: typeof HISTORICAL_ANALOG_COVERAGE_STRUCTURAL_POLICY_VERSION } { const rule=rules.find((r)=>(!r.asset||r.asset===input.asset)&&(!r.indicatorKind||r.indicatorKind===input.indicatorKind)&&(!r.indicatorCategory||r.indicatorCategory===input.indicatorCategory)&&(!r.queryStage||r.queryStage===input.queryStage)); return rule ? { structurallyUnavailable: true, reason: rule.reason, policyVersion: HISTORICAL_ANALOG_COVERAGE_STRUCTURAL_POLICY_VERSION } : { structurallyUnavailable: false, reason: null, policyVersion: HISTORICAL_ANALOG_COVERAGE_STRUCTURAL_POLICY_VERSION }; }
