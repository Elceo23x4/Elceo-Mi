...
export class MemoryHistoricalAnalogRepository implements HistoricalAnalogRepository {
  ...
  async listAnalogMemory(params: { before: string; asset?: any; limit?: number; cursor?: string }) { let rows=[...this.memory.values()].filter((r)=>Date.parse(r.availableAt)<Date.parse(params.before)); if(params.asset) rows=rows.filter((r)=>r.sourceAsset===params.asset); rows.sort((a,b)=>Date.parse(a.availableAt)-Date.parse(b.availableAt)||a.analogMemoryId.localeCompare(b.analogMemoryId)); if(params.cursor) rows=rows.filter((r)=>r.analogMemoryId>params.cursor!); return rows.slice(0, params.limit ?? 100); }
