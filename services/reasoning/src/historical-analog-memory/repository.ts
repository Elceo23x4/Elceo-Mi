...
function assertRetrievalInvariants(r: HistoricalAnalogRetrievalResult): void { const ranks=new Set<number>(); const ids=new Set<string>(); let prev=0; for (const m of r.matches) { if (m.rank <= 0) throw new Error('invalid_analog_retrieval_rank'); if (ranks.has(m.rank)) throw new Error('duplicate_analog_retrieval_rank'); if (ids.has(m.analogMemoryId)) throw new Error('duplicate_analog_retrieval_match'); if (m.rank <= prev) throw new Error('nondeterministic_analog_retrieval_rank_order'); ranks.add(m.rank); ids.add(m.analogMemoryId); prev=m.rank; } }
