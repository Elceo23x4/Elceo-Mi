...
export function getMacroSurpriseCoverageReport(asOfIso = new Date().toISOString()): MarketMacroSurpriseCoverageReport { const report: MarketMacroSurpriseCoverageReport = { generatedAt: asOfIso, representedIndicatorKinds: [...MARKET_MACRO_INDICATOR_KINDS], representedCategories: [...MARKET_MACRO_INDICATOR_CATEGORIES], pendingPhases: ['R5','R6','R7','provider_reliability'], warnings: ['consensus_dispersion_missing','historical_distribution_missing','pending_asset_direction_resolution','pending_fx_relative_strength','requires_price_confirmation','provider_activation_gap'], notes: ['C6-R4 normalizes actual vs forecast before downstream direction; R5 contradiction expansion remains pending.', 'R6 confidence calibration, R7 price reaction/impulse, and provider_reliability activation remain pending.'] }; const validation = validateMarketMacroSurpriseCoverageReport(report); if ('errors' in validation) throw new Error(`macro_surprise_coverage_invalid:${validation.errors.join('|')}`); return report; }
