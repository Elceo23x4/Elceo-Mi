import type { ConfidenceDecomposition, MarketCognitionSignal, MarketContradictionFlag, WeightedEvidenceSnapshot } from '@elceo/types';
...
export function buildConfidenceDecomposition(snapshot: WeightedEvidenceSnapshot, signals: MarketCognitionSignal[], contradictions: MarketContradictionFlag[]): ConfidenceDecomposition {
  ...
  const input=buildConfidenceCalibrationInputFromWeightedSnapshot(snapshot, { providerReliabilitySupplied:false, sourceIndependenceVerified:false });
