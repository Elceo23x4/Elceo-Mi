...
export const CFTC_LEGACY_MARKET_CODES:Readonly<Record<string,string>>={
...
export class CftcCotAdapter implements MarketEvidenceProviderAdapter {
 ...
 private async fetchInternal(request:ProviderSourceRequest,execution?:ProviderManagedExecution):Promise<ProviderSourceResponse>{
  ...
  const code=CFTC_LEGACY_MARKET_CODES[request.asset];if(!code)return fail(request,'cftc_unsupported_asset',`No canonical CFTC contract mapping for ${request.asset}`,'unsupported');
...
async function fetchLive(request:ProviderSourceRequest,code:string,cfg:ResolvedConfig,managed?:ProviderManagedExecution):Promise<ProviderSourceResponse>{
 ...
 try{const headers:Record<string,string>={'Content-Type':'application/json','Accept':'application/json'};if(cfg.appToken)headers['X-App-Token']=cfg.appToken;const response=await cfg.fetchImpl(url,{method:'POST',signal,headers,body:JSON.stringify({query,page:{pageNumber:1,pageSize:1},includeSynthetic:false})});if(!response.ok)return fail(request,response.status===429?'rate_limited':response.status>=500?'provider_5xx':'cftc_request_rejected',`CFTC request failed (${response.status})`);const json=await response.json() as unknown;const records=extractRecords(json);if(records.length===0)return{...base(request),status:'empty',sourceUrl:url,rawPayloadJson:JSON.stringify({request:cotRequest(request),rows:[]})};const row=parseRecord(records[0]!,request.asset!,code);return{...base(request),status:'success',sourceUrl:url,rawPayloadJson:JSON.stringify({request:cotRequest(request),rows:[row]})};}catch(error){if(error instanceof Error&&error.name==='AbortError')return fail(request,'cftc_timeout','CFTC request timed out');return fail(request,'cftc_fetch_error','CFTC request failed');}finally{if(timer)clearTimeout(timer);}
}
...
function parseRecord(row:RecordValue,asset:string,expectedCode:string):CotReportRow{const code=requiredString(row.cftc_contract_market_code,'cftc_contract_market_code');if(code!==expectedCode)throw new Error('cftc_contract_code_mismatch');const reportDate=requiredString(row.report_date_as_yyyy_mm_dd,'report_date_as_yyyy_mm_dd');if(Number.isNaN(Date.parse(reportDate)))throw new Error('cftc_invalid_report_date');return{reportDate,marketName:requiredString(row.market_and_exchange_names,'market_and_exchange_names'),cftcMarketCode:code,exchangeName:null,asset,openInterest:requiredNumber(row.open_interest_all,'open_interest_all'),commercialLong:nullableNumber(row.comm_positions_long_all),commercialShort:nullableNumber(row.comm_positions_short_all),nonCommercialLong:nullableNumber(row.noncomm_positions_long_all),nonCommercialShort:nullableNumber(row.noncomm_positions_short_all),nonCommercialSpreading:nullableNumber(row.noncomm_postions_spread_all),dealerLong:null,dealerShort:null,assetManagerLong:null,assetManagerShort:null,leveragedFundsLong:null,leveragedFundsShort:null,nonReportableLong:nullableNumber(row.nonrept_positions_long_all),nonReportableShort:nullableNumber(row.nonrept_positions_short_all),otherReportablesLong:null,otherReportablesShort:null};}
