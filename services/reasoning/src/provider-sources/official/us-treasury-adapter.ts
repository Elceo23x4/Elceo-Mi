...
const DATASETS=new Set(['daily_treasury_yield_curve','daily_treasury_real_yield_curve']);
export type TreasuryAdapterConfig={mode?:'fixture'|'live_disabled'|'live_enabled';fetchImpl?:typeof fetch};
export const TREASURY_XML_FIXTURE=`<?xml version="1.0" encoding="utf-8"?><feed><entry><content><m:properties xmlns:m="http://schemas.microsoft.com/ado/2007/08/dataservices/metadata" xmlns:d="http://schemas.microsoft.com/ado/2007/08/dataservices"><d:NEW_DATE>2026-06-17T00:00:00</d:NEW_DATE><d:BC_10YEAR>4.21</d:BC_10YEAR><d:TC_10YEAR>2.01</d:TC_10YEAR></m:properties></content></entry></feed>`;
...
export class UsTreasuryOfficialAdapter implements MarketEvidenceProviderAdapter{
 ...
 private async fetchInternal(request:ProviderSourceRequest,execution?:ProviderManagedExecution):Promise<ProviderSourceResponse>{
  if(request.capability!=='real_yield_series')return fail(request,'unsupported_capability');
  ...
  if(mode==='fixture')return success(request,JSON.stringify(parseTreasuryXml(TREASURY_XML_FIXTURE)),`${ORIGIN}/resource-center/data-chart-center/interest-rates/pages/xml`);
  if(mode!=='live_enabled')return fail(request,'treasury_live_disabled');
  const url=new URL('/resource-center/data-chart-center/interest-rates/pages/xml',ORIGIN);url.searchParams.set('data',params.dataset);if(params.year)url.searchParams.set('field_tdr_date_value',params.year);if(params.month)url.searchParams.set('field_tdr_date_value_month',params.month);
  try{const response=await (this.config.fetchImpl??fetch)(url,{signal:execution?.signal,headers:{Accept:'application/xml,text/xml'}});if(!response.ok)return fail(request,response.status===429?'rate_limited':`treasury_http_${response.status}`);const xml=await response.text();const parsed=parseTreasuryXml(xml);if(!parsed.length)return fail(request,'treasury_empty_or_malformed');return success(request,JSON.stringify(parsed),url.toString());}catch(error){if(error instanceof Error&&error.name==='AbortError')return fail(request,'treasury_timeout');return fail(request,'treasury_fetch_error');}
 }
 async normalize(response:ProviderSourceResponse):Promise<NormalizedMarketEvidencePayload[]>{if(!response.rawPayloadJson)return[];const rows=JSON.parse(response.rawPayloadJson) as Array<Record<string,string>>;return rows.flatMap((row,rowIndex)=>Object.entries(row).filter(([key,value])=>key!=='NEW_DATE'&&Number.isFinite(Number(value))).map(([key,value],fieldIndex)=>({payloadId:`${response.requestId}:${rowIndex}:${fieldIndex}`,evidenceTypeId:response.capability,evidenceClass:'real_yields',providerId:'us_treasury_official',sourceId:'us_treasury_official',region:'united_states',asset:null,observedAt:normalizeDate(row.NEW_DATE),publishedAt:null,normalizedAt:response.fetchedAt,confidenceScore:95,dataQuality:'high',valuesJson:JSON.stringify({field:key,value:Number(value),date:row.NEW_DATE}),metadataJson:JSON.stringify({canonicalSourceId:'us_treasury_official',gateSourceId:'us_treasury',requestId:response.requestId,sourceUrl:response.sourceUrl})})));}
}
