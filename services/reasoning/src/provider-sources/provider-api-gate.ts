...
import { assertProviderCachePolicyAuthority, buildProviderCacheIdentity, responseFromMaterial, type ProviderCacheCoordinator, type ProviderCachePolicyResolver, type ProviderCacheSnapshot } from './provider-cache/index';
...
export type ProviderCallMode = 'blocked_live'|'fixture_response'|'dry_run_no_external_call'|'replay_captured_payload'|'live_staging_call'|'live_production_call'|'cache_response';
...
export type ProviderApiGateExecutionContext = { providerControlStore?:ProviderControlStore; policyResolver?:ProviderControlPolicyResolver; credentialPoolId?:string; evaluatedAt?:number; cacheCoordinator?:ProviderCacheCoordinator; cachePolicyResolver?:ProviderCachePolicyResolver; orchestrationSignal?:AbortSignal };
export type ProviderApiGateExecutionResult = { decision:ProviderRuntimeResolverDecision; response:ProviderRuntimeResponse|null; snapshot:ProviderApiGateSnapshot; providerControlSnapshot?:ProviderControlSnapshot; cacheSnapshot?:ProviderCacheSnapshot; settlementState:ProviderSettlementState };
...
async function executeProviderApiGateRequestUncached(req:ProviderRuntimeRequest,adapter?:MarketEvidenceProviderAdapter,context:ProviderApiGateExecutionContext={}):Promise<ProviderApiGateExecutionResult> {
...
export async function executeProviderApiGateRequest(req:ProviderRuntimeRequest,adapter?:MarketEvidenceProviderAdapter,context:ProviderApiGateExecutionContext={}):Promise<ProviderApiGateExecutionResult>{
 const initial=resolveProviderRuntimeRequest(req);if(!initial.allowed||initial.providerCallMode!=='live_staging_call'||(!context.cacheCoordinator&&!context.cachePolicyResolver))return executeProviderApiGateRequestUncached(req,adapter,context);
 const pool=context.credentialPoolId,evaluatedAt=context.evaluatedAt??Date.now();if(!pool||!context.cacheCoordinator||!context.cachePolicyResolver){const decision=distributedDenial(initial,'provider_cache_control_unavailable'),response=null,snapshot=buildProviderApiGateSnapshot(decision,req.requestId,response);return {decision,response,snapshot,settlementState:'not_required'};}
 let policy;try{policy=await context.cachePolicyResolver.resolve({sourceId:req.sourceId,capabilityId:req.capabilityId,credentialPoolId:pool,evaluatedAt});if(!policy)throw new Error('provider_cache_policy_missing');assertProviderCachePolicyAuthority(policy,req,pool,evaluatedAt);}catch(error){const reason=error instanceof Error?error.message:'provider_cache_policy_missing',decision=distributedDenial(initial,reason);return {decision,response:null,snapshot:buildProviderApiGateSnapshot(decision,req.requestId,null),settlementState:'not_required'};}
 const fingerprint=buildProviderRequestFingerprint(req),identity=buildProviderCacheIdentity(req,policy,pool,fingerprint);let ownerResult:ProviderApiGateExecutionResult|undefined;const shared=await context.cacheCoordinator.execute(req,identity,policy,async signal=>{ownerResult=await executeProviderApiGateRequestUncached(req,adapter,{...context,orchestrationSignal:signal});return {response:ownerResult.response,settlementState:ownerResult.settlementState,result:ownerResult};});
 if(shared.material){const response=responseFromMaterial(shared.material,req),decision={...initial,providerCallMode:'cache_response' as const,cacheStatus:'hit' as const,reason:shared.freshness==='stale'?'stale_if_error':'cache_hit'},now=Date.now(),cacheSnapshot:ProviderCacheSnapshot={cacheKeyHash:`sha256:${identity.hash}`,cacheLayer:shared.layer,freshness:shared.freshness,ageMs:shared.entry?Math.max(0,now-shared.entry.publishedAt):null,freshUntil:shared.entry?.freshUntil??null,staleUntil:shared.entry?.staleUntil??null,singleFlightRole:shared.role,singleFlightOutcome:shared.freshness==='stale'?'stale_if_error':'success',cachePolicyVersion:policy.policyVersion};return {decision,response,snapshot:buildProviderApiGateSnapshot(decision,req.requestId,response),cacheSnapshot,settlementState:'not_required'};}
 if(ownerResult)return ownerResult;const decision=distributedDenial(initial,shared.failureReason??'provider_cache_control_unavailable');return {decision,response:null,snapshot:buildProviderApiGateSnapshot(decision,req.requestId,null),cacheSnapshot:{cacheKeyHash:`sha256:${identity.hash}`,cacheLayer:'none',freshness:'miss',ageMs:null,freshUntil:null,staleUntil:null,singleFlightRole:shared.role,singleFlightOutcome:shared.failureReason??'failure',cachePolicyVersion:policy.policyVersion},settlementState:'not_required'};
}
