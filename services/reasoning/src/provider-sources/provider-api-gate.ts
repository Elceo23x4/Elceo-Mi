...
export function buildProviderNormalizedRequestKey(req: ProviderRuntimeRequest): string { return [req.sourceId, req.capabilityId, req.asset ?? 'none', req.region ?? 'none', req.startAt ?? 'none', req.endAt ?? 'none', req.paginationCursor ?? 'none'].join('|'); }
...
export async function executeProviderApiGateRequest(req:ProviderRuntimeRequest, adapter?:MarketEvidenceProviderAdapter):Promise<{decision:ProviderRuntimeResolverDecision; response:ProviderRuntimeResponse|null; snapshot:ProviderApiGateSnapshot}> { const decision=resolveProviderRuntimeRequest(req); let response:ProviderRuntimeResponse|null=null; if(!decision.allowed){ response = req.policy?.fallbackMode === 'stale_if_error' && req.policy.stalePayload ? req.policy.stalePayload : null; } else if(req.policy?.cacheHitPayload){ response=req.policy.cacheHitPayload; } else if(decision.providerCallMode==='replay_captured_payload'){ response=req.replayPayload??null; } else if(decision.providerCallMode==='dry_run_no_external_call'){ response={requestId:req.requestId,responseId:`dry-${req.requestId}`,sourceId:req.sourceId,capabilityId:req.capabilityId,adapterId:decision.adapterId!,receivedAt:new Date().toISOString(),payload:{dryRun:true, normalizedRequestKey:decision.normalizedRequestKey},payloadSchemaStatus:'valid',payloadSizeBytes:15,recordCount:0,provenance:{requestId:req.requestId,sourceId:req.sourceId},error:null,rateLimit:null}; } else if(decision.providerCallMode==='live_production_call'){ response=errorResponse(req, decision, 'production_live_not_approved'); } else if(decision.providerCallMode==='live_staging_call' && adapter){ const sourceReq:ProviderSourceRequest={requestId:req.requestId,providerId:req.sourceId,capability:req.capabilityId,asset:req.asset??null,region:req.region??null,evidenceTypeId:req.capabilityId,requestedAt:new Date().toISOString(),paramsJson:JSON.stringify(redactProviderSecrets(req.metadata??{}))}; const res:ProviderSourceResponse=await adapter.fetch(sourceReq); response={requestId:req.requestId,responseId:`staging-${req.requestId}`,sourceId:req.sourceId,capabilityId:req.capabilityId,adapterId:decision.adapterId!,receivedAt:res.fetchedAt,payload:res.rawPayloadJson?JSON.parse(res.rawPayloadJson):null,payloadSchemaStatus:res.status==='success'?'valid':res.errorCode==='rate_limited'?'rate_limited':'provider_error',payloadSizeBytes:(res.rawPayloadJson??'').length,recordCount:0,provenance:{requestId:req.requestId,sourceId:req.sourceId},error:res.errorCode?{category:res.errorCode,message:redactProviderSecrets(res.errorMessage??'')}:null,rateLimit:res.errorCode==='rate_limited'?{remaining:0,resetAt:null}:null}; } else if(decision.providerCallMode==='live_staging_call'){ response=errorResponse(req, decision, 'managed_adapter_required'); } else if(decision.providerCallMode==='fixture_response' && adapter){ const sourceReq:ProviderSourceRequest={requestId:req.requestId,providerId:req.sourceId,capability:req.capabilityId,asset:req.asset??null,region:req.region??null,evidenceTypeId:req.capabilityId,requestedAt:new Date().toISOString(),paramsJson:'{}'}; const res:ProviderSourceResponse=await adapter.fetch(sourceReq); response={requestId:req.requestId,responseId:`fixture-${req.requestId}`,sourceId:req.sourceId,capabilityId:req.capabilityId,adapterId:decision.adapterId!,receivedAt:res.fetchedAt,payload:res.rawPayloadJson?JSON.parse(res.rawPayloadJson):null,payloadSchemaStatus:res.status==='success'?'valid':res.errorCode==='rate_limited'?'rate_limited':'provider_error',payloadSizeBytes:(res.rawPayloadJson??'').length,recordCount:0,provenance:{requestId:req.requestId,sourceId:req.sourceId},error:res.errorCode?{category:res.errorCode,message:redactProviderSecrets(res.errorMessage??'')}:null,rateLimit:res.errorCode==='rate_limited'?{remaining:0,resetAt:null}:null}; } if(response) response=validateProviderRuntimeResponse(response,decision,req.policy); const snapshot=buildProviderApiGateSnapshot(decision,req.requestId,response,response?.error?.category); return {decision,response,snapshot}; }
