...
import { buildProviderRequestFingerprint, validateProviderControlPolicy, type ProviderControlPolicy, type ProviderControlStore, type ProviderReservation } from './provider-control/index';
...
export type ProviderApiGateExecutionContext = { providerControlStore?:ProviderControlStore };
export async function executeProviderApiGateRequest(req:ProviderRuntimeRequest, adapter?:MarketEvidenceProviderAdapter, context:ProviderApiGateExecutionContext={}):Promise<{decision:ProviderRuntimeResolverDecision; response:ProviderRuntimeResponse|null; snapshot:ProviderApiGateSnapshot}> {
  let decision=resolveProviderRuntimeRequest(req); let response:ProviderRuntimeResponse|null=null; let reservation:ProviderReservation|undefined; let providerInvoked=false;
  if(decision.allowed && decision.providerCallMode==='live_staging_call') {
    if(!req.providerControlPolicy){decision={...decision,allowed:false,reason:'provider_control_policy_missing',providerCallMode:'blocked_live',observabilityLabels:{reason:'provider_control_policy_missing'}};}
    else if(!context.providerControlStore || context.providerControlStore.kind!=='redis') {decision={...decision,allowed:false,reason:'provider_control_unavailable',providerCallMode:'blocked_live',observabilityLabels:{reason:'provider_control_unavailable'}};}
    else { try { validateProviderControlPolicy(req.providerControlPolicy); const admission=await context.providerControlStore.admit({requestId:req.requestId,ownerToken:randomUUID(),fingerprint:buildProviderRequestFingerprint(req),policy:req.providerControlPolicy}); if(!admission.allowed) decision={...decision,allowed:false,reason:admission.reason,providerCallMode:'blocked_live',observabilityLabels:{reason:admission.reason}}; else reservation=admission.reservation; } catch { decision={...decision,allowed:false,reason:'provider_control_unavailable',providerCallMode:'blocked_live',observabilityLabels:{reason:'provider_control_unavailable'}}; } }
  }
  try {
    ...
    if(response) response=validateProviderRuntimeResponse(response,decision,req.policy);
    if(reservation) await context.providerControlStore!.settle(reservation,response?.payloadSchemaStatus==='valid'?'COMMITTED':providerInvoked?'COMMIT_REQUIRED_UNKNOWN_OUTCOME':'RELEASED');
  } catch(error) { if(reservation) await context.providerControlStore!.settle(reservation,providerInvoked?'COMMIT_REQUIRED_UNKNOWN_OUTCOME':'RELEASED'); throw error; }
  const snapshot=buildProviderApiGateSnapshot(decision,req.requestId,response,response?.error?.category); return {decision,response,snapshot};
}
