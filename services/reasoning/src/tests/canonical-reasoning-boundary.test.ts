...
function buildAssembly(asOf: string, priorSnapshotId: string | null): ReasoningInputAssemblyResult {
  return {
    input: buildReasoningInputFrameFixture({ asOf, events: [buildCanonicalEventFixture({ id: 'evt-boundary' })] }),
    sourceRunId: 'ing-1',
...
export async function runCanonicalReasoningBoundaryTests(): Promise<void> {
  ...
  const successService = new CanonicalReasoningBoundaryService(
    new StubAssembler(buildAssembly('2026-04-22T11:00:00.000Z', null)) as never,
    { evaluate: () => buildCanonicalCognitionStateFixture({ evaluatedAt: '2026-04-22T11:00:00.000Z' }) },
  ...
  assert(success.drift === null, 'first successful run with no prior snapshot should return null drift');

  const secondService = new CanonicalReasoningBoundaryService(
    new StubAssembler(buildAssembly('2026-04-22T11:05:00.000Z', success.report.snapshotId)) as never,
    { evaluate: () => buildCanonicalCognitionStateFixture({ evaluatedAt: '2026-04-22T11:05:00.000Z', bias: 'neutral' }) },
    { engineName: 'mock-engine', reasoningVersion: 'r1', scoringVersion: 's1' },
    repo
  );
  const second = await secondService.executeAssetWindow({ asset: 'XAU/USD', timeframe: 'H1', asOf: '2026-04-22T11:05:00.000Z' });
  assert(second.report.status === 'success', 'second run should remain successful when drift persistence works');
  assert(second.drift !== null, 'second run with prior snapshot should build and return drift report');

  const persistedDrift = second.drift ? await repo.driftRepository.getDriftById(second.drift.driftId) : null;
  assert(persistedDrift !== null, 'drift report should be persisted when comparison succeeds');
  ...
  const engineThrowService = new CanonicalReasoningBoundaryService(
    new StubAssembler(buildAssembly('2026-04-22T11:10:00.000Z', null)) as never,
    { evaluate: () => { throw new Error('boom'); } },
    { engineName: 'mock-engine', reasoningVersion: 'r1', scoringVersion: 's1' },
    repo
  );
  const engineFail = await engineThrowService.executeAssetWindow({ asset: 'XAU/USD', timeframe: 'H1', asOf: '2026-04-22T11:10:00.000Z' });
  ...
  const invalidService = new CanonicalReasoningBoundaryService(
    new StubAssembler(buildAssembly('2026-04-22T11:15:00.000Z', null)) as never,
    { evaluate: () => ({ bad: true } as never) },
    { engineName: 'mock-engine', reasoningVersion: 'r1', scoringVersion: 's1' },
    repo
  );
  const invalid = await invalidService.executeAssetWindow({ asset: 'XAU/USD', timeframe: 'H1', asOf: '2026-04-22T11:15:00.000Z' });
  ...
  const partialService = new CanonicalReasoningBoundaryService(
    new StubAssembler(buildAssembly('2026-04-22T11:20:00.000Z', null)) as never,
    { evaluate: () => buildCanonicalCognitionStateFixture({ evaluatedAt: '2026-04-22T11:20:00.000Z' }) },
    { engineName: 'mock-engine', reasoningVersion: 'r1', scoringVersion: 's1' },
    failingSnapshotRepo
  );
  const partial = await partialService.executeAssetWindow({ asset: 'XAU/USD', timeframe: 'H1', asOf: '2026-04-22T11:20:00.000Z' });
  ...
  const driftFailureRepo = new MemoryReasoningPersistenceRepository(
    repo.runRepository,
    repo.snapshotRepository,
    {
      ...repo.driftRepository,
      saveDriftRecord: async () => {
        throw new Error('drift fail');
      }
    }
  );
  const driftFailureService = new CanonicalReasoningBoundaryService(
    new StubAssembler(buildAssembly('2026-04-22T11:25:00.000Z', second.report.snapshotId)) as never,
    { evaluate: () => buildCanonicalCognitionStateFixture({ evaluatedAt: '2026-04-22T11:25:00.000Z', bias: 'bearish' }) },
    { engineName: 'mock-engine', reasoningVersion: 'r1', scoringVersion: 's1' },
    driftFailureRepo
  );
  const driftFailure = await driftFailureService.executeAssetWindow({ asset: 'XAU/USD', timeframe: 'H1', asOf: '2026-04-22T11:25:00.000Z' });
  assert(driftFailure.report.status === 'partial_success', 'drift persistence failure should downgrade status to partial_success');
  assert((driftFailure.report.failureReason ?? '').startsWith('drift_persistence_failure:'), 'drift failure reason should be explicit');
  ...
  const defaultEngineService = createCanonicalReasoningBoundaryService({
    assembler: new StubAssembler(buildAssembly('2026-04-22T11:00:00.000Z', null)) as never,
