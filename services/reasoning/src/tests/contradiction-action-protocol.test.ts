...
async function environment(options: { evaluation?: Record<string, unknown>; reliability?: 'verified'|'fixture'|'unverified'; invalidation?: InvalidationState; contradictionInput?: MarketContradictionInput; preEvaluatedAt?:string; pointProvenance?:Array<{sourceId:string;contentHash:string;reliability:'verified'|'replay'|'fixture'|'unverified';verificationRef?:string;verifiedAt?:string}>; sourceEvidenceIds?:string[] } = {}) {
  ...
  const service = new ContradictionActionProtocolService({ eventRealityRepository, eventExpectationRepository, cognitionSnapshotRepository, contradictionInputRepository, protocolRepository });
  return { service, protocolRepository, contradictionInputRepository, eventRealityRepository, eventExpectationRepository, cognitionSnapshotRepository };
}
