...
import { ContradictionActionProtocolService, MemoryContradictionActionProtocolRepository, MemoryPersistedContradictionInputRepository, SqlContradictionActionProtocolRepository, normalizePersistedContradictionInputRecord, payloadHash, validateNoAdviceRecord } from '../contradiction-action-protocol/index.js';
...
async function environment(options: { evaluation?: Record<string, unknown>; reliability?: 'verified'|'fixture'|'unverified'; invalidation?: InvalidationState; contradictionInput?: MarketContradictionInput; preEvaluatedAt?:string } = {}) {
  ...
  await contradictionInputRepository.saveContradictionInput(normalizePersistedContradictionInputRecord({recordId:'',eventEvaluationId:assessed.eventEvaluationId,expectationId:expected.expectationId,asset:'xau_usd',assessmentStage:assessed.assessmentStage as never,assessmentEvidenceHash:assessed.assessmentEvidenceHash,availableAt:'2026-01-01T01:00:00.000Z',evidenceCutoffAt:cutoff,input:persistedInput,normalizedInputHash:'',sourceEvidenceIds:['source-1'],provenance:[{sourceId:'source-1',contentHash:'source-hash',reliability}],providerReliabilitySupplied:persistedInput.providerReliabilitySupplied,sourceIndependenceVerified:persistedInput.sourceIndependenceVerified,warnings:persistedInput.warnings,limitations:[],createdAt:'2026-01-01T01:00:00.000Z',canonicalPayloadHash:''}));
...
export async function runContradictionActionProtocolTests(): Promise<void> {
  ...
  assert(['review_required','escalate_review'].includes((await decide({ evaluation:{ outcome:'reversed' },contradictionInput:input({evidencePoints:[]}) })).protocolState), 'final reversal reviews or escalates according to direct severity');
  assert(['review_required','escalate_review'].includes((await decide({ evaluation:{ outcome:'mispriced_candidate' },contradictionInput:input({evidencePoints:[]}) })).protocolState), 'final mispriced candidate reviews or escalates according to direct severity');
