...
export async function runEvidenceFabricTests():Promise<void> {
 ...
 assert.deepEqual(EVIDENCE_EXECUTION_REGISTRATIONS.map(x=>`${x.sourceId}:${x.capabilityId}`),['tiingo_market_data:direct_price','fred_macro:real_yields','fred_macro:financial_conditions','ecb_official:ecb_policy','us_treasury_official:treasury_yields','us_treasury_official:real_yields']);
 ...
 const fredProjection=projectOfficialMacroVintage({
  request:{requestId:'fred-r1',providerId:'fred',capability:'real_yield_series',paramsJson:JSON.stringify({seriesId:'DFII10'}),requestedAt:'2026-06-17T12:00:00.000Z'},
