...
export async function runEvidenceFabricTests():Promise<void> {
 ...
 assert.deepEqual(EVIDENCE_EXECUTION_REGISTRATIONS.map(x=>`${x.sourceId}:${x.capabilityId}`),['tiingo_market_data:direct_price']);
 assert.equal(evidenceImplementationState('fred_macro'),'fixture_parser_ready');assert.equal(evidenceImplementationState('ecb_official'),'fixture_parser_ready');assert.equal(evidenceImplementationState('us_treasury_official'),'fixture_parser_ready');
 ...
 assert.equal(resolveEvidenceGateExecution('us_treasury_official','treasury_yields'),null,'Treasury parser must remain outside executable routing until the Provider API Gate has a semantically correct nominal-yield capability');
