...
describe('official macro sources', () => {
  ...
  it('boundary exposes methods', () => { const reg = boundary.getOfficialMacroSourceRegistry(); assert.ok(reg.length >= 14); const src = reg[0]; assert.ok(src); assert.ok(boundary.getOfficialMacroSourceDescriptor(src.sourceId)); const sourceFixtures = listOfficialMacroFixturePayloads(src.sourceId); assert.ok(sourceFixtures.length > 0); const fixture = getOfficialMacroFixturePayload(src.sourceId, sourceFixtures[0]!.fixtureId); assert.ok(fixture); });
});
