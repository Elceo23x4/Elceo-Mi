...
export async function runProviderApiGateTests(){
  ...
  const liveRequest=base({ activationMode:'staging_live_allowed', policy:{ explicitStagingLiveAllow:true,requestMetadata:{credentialPresent:true} } });
  const missingCachePolicy=await executeProviderApiGateRequest(liveRequest,countingAdapter);assert.equal(missingCachePolicy.decision.reason,'provider_cache_policy_missing');assert.equal(adapterCalls,0);
  const requestPolicyIgnored=await executeProviderApiGateRequest({...liveRequest,providerControlPolicy:{} as never},countingAdapter);assert.equal(requestPolicyIgnored.decision.reason,'provider_cache_policy_missing');assert.equal(adapterCalls,0);
  const missingCoordinator=await executeProviderApiGateRequest(liveRequest,countingAdapter,{cachePolicyResolver:{resolve:async()=>null},credentialPoolId:'primary'});assert.equal(missingCoordinator.decision.reason,'provider_cache_control_unavailable');assert.equal(adapterCalls,0);
