...
export async function runProviderApiGateTests(){
  ...
  const requestPolicyIgnored=await executeProviderApiGateRequest(base({ activationMode:'staging_live_allowed', policy:{ explicitStagingLiveAllow:true,requestMetadata:{credentialPresent:true} },providerControlPolicy:buildTestProviderControlPolicy() }),countingAdapter);assert.equal(requestPolicyIgnored.decision.reason,'provider_control_policy_missing');assert.equal(adapterCalls,0);
  const approved=buildTestProviderControlPolicy();const resolver={resolve:async()=>approved};const unavailable=await executeProviderApiGateRequest(base({ activationMode:'staging_live_allowed',policy:{explicitStagingLiveAllow:true,requestMetadata:{credentialPresent:true}}}),countingAdapter,{policyResolver:resolver,credentialPoolId:'primary'});assert.equal(unavailable.decision.reason,'provider_control_unavailable');assert.equal(adapterCalls,0);
  const invalidLease=buildTestProviderControlPolicy({concurrency:{maxConcurrent:1,leaseDurationMs:500,providerTimeoutMs:300}});const invalidLeaseResult=await executeProviderApiGateRequest(base({activationMode:'staging_live_allowed',policy:{explicitStagingLiveAllow:true,requestMetadata:{credentialPresent:true}}}),countingAdapter,{policyResolver:{resolve:async()=>invalidLease},credentialPoolId:'primary'});assert.equal(invalidLeaseResult.decision.reason,'provider_control_lease_timeout_invariant');
  const testOnly=buildTestProviderControlPolicy({status:'test_only'});const notApproved=await executeProviderApiGateRequest(base({activationMode:'staging_live_allowed',policy:{explicitStagingLiveAllow:true,requestMetadata:{credentialPresent:true}}}),countingAdapter,{policyResolver:{resolve:async()=>testOnly},credentialPoolId:'primary'});assert.equal(notApproved.decision.reason,'provider_control_policy_not_approved');assert.equal(adapterCalls,0);
  const memory=new MemoryProviderControlStore();const redisLike:ProviderControlStore={kind:'redis',isReady:()=>true,admit:r=>memory.admit(r),claimExecution:(r,t)=>memory.claimExecution(r,t),settle:(r,status)=>memory.settle(r,status),close:()=>memory.close()};const preExecution=await executeProviderApiGateRequest(base({requestId:'pre-execution',activationMode:'staging_live_allowed',policy:{explicitStagingLiveAllow:true,requestMetadata:{credentialPresent:true}}}),undefined,{policyResolver:resolver,credentialPoolId:'primary',providerControlStore:redisLike});assert.equal(preExecution.settlementState,'settled_released');const next=await redisLike.admit({admissionId:'next',reservationId:'next-r',ownerToken:'next-o',requestId:'next',fingerprint:'next-f',policy:approved});assert.equal(next.allowed,true);
  const failingSettlement:ProviderControlStore={...redisLike,settle:async()=>false};const unconfirmed=await executeProviderApiGateRequest(base({requestId:'unconfirmed',activationMode:'staging_live_allowed',policy:{explicitStagingLiveAllow:true,requestMetadata:{credentialPresent:true}}}),undefined,{policyResolver:resolver,credentialPoolId:'primary',providerControlStore:failingSettlement});assert.equal(unconfirmed.settlementState,'settlement_unconfirmed');
